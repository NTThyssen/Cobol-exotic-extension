/*
 ...
 The authors
 accept no liability for damages of any kind resulting from the use
 of this software. Use at your
 own risk.
 ...
 Db2 for z/OS Version 12 documentation served as the original
 source 
 material.
 
 The ALTER FUNCTION variations (external), (inlined SQL scalar), and
 (SQL
 table) are all variations on each other and are contained in
 the alterFunctionStatement rule.
 
 The rule signalStatement is not a full implementation of the syntax
 of the SIGNAL statement, but a
 subset that is possible to embed in
 an application program. Look to sqlplSignalStatement for the
 SQL/PL
 implementation.
 
 The rule trustedContextOptionList does not strictly match its
 syntax
 diagram, for reasons documented with the rule.
 
 The rule executeImmediateStatement only partially
 implements the
 syntax of the EXECUTE IMMEDIATE statement in order to avoid
 implementing a grammar
 for the entire PL/I language here. The same
 is true of the prepareStatement rule.
 
 I do not know
 if the SQL statement 
 ALTER PROCEDURE (SQL - external)
 will work with this grammar. It's been
 deprecated but the syntax
...
/*
 ...
 releaseConnectionStatement rules is significant. In
 order
 ...
 */
cursorName: hostIdentifier;

statementName: identifier;

descriptorName: hostVariable;

holdability: ((WITHOUT HOLD) | (WITH HOLD));
...
notNullPhrase: ((NOT NULL) | (NOT NULL WITH DEFAULT));
...
alterFunctionStatementExternal: (
		ALTER functionDesignator1 functionExternalOptionList+
	);

alterFunctionStatementCompiledSqlScalar: (
		ALTER functionDesignator2 (
			(
				ALTER? alterWhichFunction1? functionCompiledSqlScalarOptionList+
			)
			| (
				REPLACE alterWhichFunction2? functionCompiledSqlScalarRoutineSpecification
			)
			| (
				ADD versionOption functionCompiledSqlScalarRoutineSpecification
			)
...
alterWhichFunction2: ((ACTIVE VERSION) | versionOption);

functionCompiledSqlScalarRoutineSpecification: (
		LPAREN (
			parameterDeclaration2 (COMMA parameterDeclaration2)*
		)? RPAREN functionReturnsClause functionCompiledSqlScalarOptionList* sqlRoutineBody
	);

alterFunctionStatementInlineSqlScalar: (
		ALTER functionDesignator2 functionInlineSqlScalarOptionList+
	);

alterFunctionStatementSqlTable: (
		ALTER functionDesignator2 RESTRICT functionSqlTableOptionList+
	);

functionReturnsClause: (RETURNS functionDataType);

functionDesignator1: (
		(
			(
				FUNCTION functionName (
					LPAREN functionParameterType (
						COMMA functionParameterType
					)* RPAREN
				)?
			)
...
functionDesignator2: (
		(
			(
				FUNCTION functionName (
					LPAREN functionDataType (
						COMMA functionDataType
					)* RPAREN
				)?
			)
			| (SPECIFIC FUNCTION specificName)
 ...
 */
alterIndexStatement: (
		ALTER INDEX indexName regenerateClause? alterIndexOptions* (
			alterIndexPartitionOptions (
				COMMA? alterIndexPartitionOptions
			)*
		)?
	);
		...
		ALTER PROCEDURE procedureName (
			...
			| (
				REPLACE alterWhichProcedureSQLPL2? (
					LPAREN parameterDeclaration3 (
						COMMA parameterDeclaration3
					)* RPAREN
				)? procedureSQLPLOptionList* sqlRoutineBody
			)
			| (
				ADD versionOption (
					LPAREN parameterDeclaration3 (
...
alterWhichProcedureSQLPL2: ((ACTIVE VERSION) | versionOption);
...
/*
 ...
 ? (0 or 1 allowed) to * (0 or
 more allowed) per documentation.
 Noticed whilst working on issue 125.
 */
alterTablespaceStatement: (
		ALTER TABLESPACE (databaseName DOT)? tablespaceName alterTablespaceOptionList*
			alterPartitionClause* moveTableClause?
	);

alterTriggerStatement: ( ALTER TRIGGER triggerName NOT? SECURED);
...
alterWhichTrigger2: ((ACTIVE VERSION) | versionOption);
...
triggerSpecification: (
		triggerAdvancedActivationTime triggerEvent ON tableName triggerAdvancedReferencingPhrase?
...
associateLocatorsStatement: (
		ASSOCIATE (RESULT SET)? (LOCATOR | LOCATORS) LPAREN rsLocatorVariable (
			COMMA rsLocatorVariable
		)* RPAREN WITH PROCEDURE (procedureName | hostVariable)
	);

beginDeclareSectionStatement: (BEGIN DECLARE SECTION);
...
dsnutilUCSDsn: (
		...
		| (
			DSNUTIL_DSN_LPAREN (
				...
				| DSNUTIL_PAREN_DOT
			)+ dsnutilUCSDsnMemberName? DSNUTIL_RPAREN1
		)
		| (
			DSNUTIL_DSN_LPAREN DSNUTIL_PAREN_OPEN_APOS DSNUTIL_APOS_CHAR+ dsnutilUCSDsnMemberName?
				DSNUTIL_APOS DSNUTIL_RPAREN1
		)
		| (
		...
		| (
			DSNUTIL_DSN_LPAREN DSNUTIL_DSN_DOUBLE_APOS DSNUTIL_DOUBLE_APOS_CHAR+
				DSNUTIL_DOUBLE_APOS1 DSNUTIL_RPAREN1
		)
		| (
			DSNUTIL_DSN_LPAREN DSNUTIL_PAREN_DOUBLE_APOS DSNUTIL_DOUBLE_APOS_CHAR+
				DSNUTIL_DOUBLE_APOS1 DSNUTIL_RPAREN1
		)
		| (
			DSNUTIL_DSN_WS_LPAREN DSNUTIL_PAREN_DOUBLE_APOS DSNUTIL_DOUBLE_APOS_CHAR+
				DSNUTIL_DOUBLE_APOS1 DSNUTIL_RPAREN1
		)
		| (
			DSNUTIL_DSN_WS_LPAREN DSNUTIL_PAREN_OPEN_APOS DSNUTIL_APOS_CHAR+ DSNUTIL_APOS
				DSNUTIL_RPAREN1
		)
		| (
			DSNUTIL_DSN_WS_LPAREN DSNUTIL_PAREN_OPEN_QUOTE DSNUTIL_QUOTE_CHAR DSNUTIL_QUOTE1
				DSNUTIL_RPAREN1
		)
	);
...
dsnutilUCSReplicationCopyOption: (
		...
		| (
			DSNUTIL_DUMPONLY (
				DSNUTIL_TOKEN dsnutilUCSArgInParens
			)? dsnutilUCSDumpclassSpec?
		)
	);

dsnutilUCSDumpclassSpec: (DSNUTIL_DUMPCLASS dsnutilUCSArgList1);
...
dsnutilUCSUnlddnSpec: (
		DSNUTIL_UNLDDN (
			...
			| (
				...
				) dsnutilUCSCatmaintActionToken DSNUTIL_RPAREN1
			)
		)
	);
...
dsnutilUCSXMLTablespacePhrase: (
		DSNUTIL_INCLUDE DSNUTIL_XML DSNUTIL_TABLESPACES (
			DSNUTIL_ALL
			| dsnutilUCSXMLSpec
		)?
	);
...
dsnutilUCSXMLSpec: (
		DSNUTIL_LPAREN dsnutilUCSXMLSpecTableOrTablespace (
			DSNUTIL_COMMA dsnutilUCSXMLSpecTableOrTablespace
...
dsnutilUCSXMLSpecTableOrTablespace: (
		...
		| (
			DSNUTIL_PAREN_TABLE dsnutilUCSQualifiedTableName DSNUTIL_PAREN_XMLCOLUMN
...
dsnutilUCSCloneOption: (DSNUTIL_CLONE);
		...
		DSNUTIL_SHRLEVEL (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_REFERENCE | DSNUTIL_CHANGE) DSNUTIL_RPAREN1
			)
		)
	);
...
/*
 ...
 The parentheses are not in the syntax
 diagram in IBM's documentation
		...
		DSNUTIL_AUXERROR (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_REPORT | DSNUTIL_INVALIDATE) DSNUTIL_RPAREN1
			)
		)
	);
		...
		DSNUTIL_DELETE (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) DSNUTIL_YES DSNUTIL_RPAREN1 dsnutilUCSLogOption
			)
		)
	);
...
/*
 ...
 parentheses are not in the syntax
 diagram in IBM's documentation
 as of this date but are tolerated by DB2.
 */
dsnutilUCSYesOrNo: (
		(
			(
				...
				| DSNUTIL_LPAREN1
				| DSNUTIL_DB_TS_LPAREN
			) (DSNUTIL_YES | DSNUTIL_NO) DSNUTIL_RPAREN1
		)
		| DSNUTIL_YES
		| DSNUTIL_NO
	);

dsnutilUCSLogOption: (DSNUTIL_LOG dsnutilUCSYesOrNo);
...
dsnutilUCSCheckIndex: (
		(
			...
			| (
				DSNUTIL_CHECK_INDEX_OPEN_PAREN dsnutilUCSIndexPhrase (
					DSNUTIL_COMMA dsnutilUCSIndexPhrase
				)* DSNUTIL_RPAREN1
			)
			| (
...
dsnutilUCSCheckLob: (
		DSNUTIL_CHECK_LOB dsnutilUCSTablespacePhrase dsnutilUCSCheckLobOptions*
	);
...
dsnutilUCSCopy: (
		...
		) dsnutilUCSCloneOption? dsnutilUCSShrlevelOption? dsnutilUCSScopeOption?
	);

/*
 This does not match the syntax diagram, as there are
 discrepencies between the syntax diagram and
 the examples.
 
 TODO Maybe rewrite this in a simpler form where it's just
 all the
 dsnutilUCSCopySpecOptions and the contents of this
 rule "or'd" together.
 */
dsnutilUCSCopySpec:
	dsnutilUCSCopySpecOptions* (
		(
			dsnutilUCSListNameWithLit dsnutilUCSCopySpecOptions* dsnutilUCSDatasetSpec?
		...
		DSNUTIL_DSNUM (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_ALL | dsnutilUCSArg) DSNUTIL_RPAREN1
			)
		)
	);
		...
		DSNUTIL_FLASHCOPY (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) DSNUTIL_NO DSNUTIL_RPAREN1
			)
			| (
				(DSNUTIL_YES | DSNUTIL_CONSISTENT) (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_YES | DSNUTIL_CONSISTENT) DSNUTIL_RPAREN1 (
					DSNUTIL_FCCOPYDDN dsnutilUCSArgInParens
				)?
			)
		)
...
dsnutilUCSFullOrChangelimit: (
		(DSNUTIL_FULL dsnutilUCSYesOrNo)
		...
		DSNUTIL_COPYTOCOPY (
			...
			| (
				(
					dsnutilUCSTablespaceCopySpec
					| dsnutilUCSIndexspaceCopySpec
				) dsnutilUCSFromCopySpec? dsnutilUCSDatasetSpec?
			)+
		) DSNUTIL_CLONE?
	);
...
dsnutilUCSFromCopyDsnOption: (
		DSNUTIL_FROMCOPY dsnutilUCSDsn (
			DSNUTIL_FROMVOLUME (
				DSNUTIL_CATALOG
				| (
					(
						...
						| DSNUTIL_LPAREN1
						| DSNUTIL_DB_TS_LPAREN
					) DSNUTIL_CATALOG DSNUTIL_RPAREN1
				)
				| (
					dsnutilUCSArg (
						DSNUTIL_FROMSEQNO dsnutilUCSArgOptionalParens
					)?
				)
				| (
					(
...
dsnutilUCSDiagnoseMessageOrTraceID: (
		(
			DSNUTIL_MESSAGE dsnutilUCSArgOptionalParens (
				DSNUTIL_INSTANCE dsnutilUCSArgOptionalParens
			)?
		)
		| (
			DSNUTIL_TRACEID dsnutilUCSArgOptionalParens (
				DSNUTIL_INSTANCE dsnutilUCSArgOptionalParens
			)?
		)
	);
...
dsnutilUCSListdef: (
		DSNUTIL_LISTDEF dsnutilUCSListName dsnutilUCSListdefContents+
	);
...
dsnutilUCSListdefTablespacesOption: ( DSNUTIL_TABLESPACES);
...
dsnutilUCSListdefClonedOption: (DSNUTIL_CLONED dsnutilUCSYesOrNo);
		...
		DSNUTIL_DEFINED (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_YES | DSNUTIL_NO | DSNUTIL_ALL) DSNUTIL_RPAREN1
			)
		)
	);

dsnutilUCSListdefRIOption: ( DSNUTIL_RI);
...
dsnutilUCSListdefBasicOption: ( DSNUTIL_BASIC dsnutilUCSYesOrNo);
...
/*
 ...
 precede and succeed the into-table-spec.
 This is contrary
 to the syntax diagram.
 */
dsnutilUCSLoad: (
		(DSNUTIL_LOAD | DSNUTIL_LOAD_DATA) dsnutilUCSLoadOptions* dsnutilUCSIntoTableSpec+
...
dsnutilUCSPreformatOption: ( DSNUTIL_PREFORMAT);
...
dsnutilUCSLoadPresortOption: ( DSNUTIL_PRESORT);
		...
		DSNUTIL_ROWFORMAT (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_RRF | DSNUTIL_BRF) DSNUTIL_RPAREN1
			)
		)
	);
		...
		DSNUTIL_RBALRSN_CONVERSION (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_EXTENDED | DSNUTIL_NONE) DSNUTIL_RPAREN1
			)
		)
	);

dsnutilUCSLoadResumeOption: ( dsnutilUCSLoadResumeSpec);

dsnutilUCSLoadKeepdictionaryOption: ( DSNUTIL_KEEPDICTIONARY);

dsnutilUCSReuseOption: ( DSNUTIL_REUSE);

dsnutilUCSLoadLogOption: ( DSNUTIL_LOG dsnutilUCSYesOrNo);

/*
 Made into separate option per Martijn Rutte 2023-22-10.
 */
dsnutilUCSLoadNocopypendOption: ( DSNUTIL_NOCOPYPEND);

dsnutilUCSLoadWorkddnOption: ( dsnutilUCSWorkddnSpec);
		...
		DSNUTIL_SORTKEYS (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_NO | dsnutilUCSArg) DSNUTIL_RPAREN1
			)
		)?
	);

dsnutilUCSLoadFloatOption: (DSNUTIL_FLOAT dsnutilUCSArgInParens);
...
dsnutilUCSCCSIDOption: ( DSNUTIL_CCSID dsnutilUCSArgList1);

dsnutilUCSCCSIDPhrase: (
		DSNUTIL_CCSID dsnutilUCSArgOptionalParens
	);

dsnutilUCSNosubsOption: ( DSNUTIL_NOSUBS);
		...
		DSNUTIL_ENFORCE (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_CONSTRAINTS | DSNUTIL_NO) DSNUTIL_RPAREN1
			)
		)
	);

dsnutilUCSNocheckpendOption: ( DSNUTIL_NOCHECKPEND);
...
dsnutilUCSBackoutOption: ( DSNUTIL_BACKOUT dsnutilUCSYesOrNo?);

dsnutilUCSLoadContinueifOption: (
		DSNUTIL_CONTINUEIF dsnutilUCSArgInParens DSNUTIL_EQUAL dsnutilUCSArg
	);

dsnutilUCSLoadIgnoreOption: ( dsnutilUCSIgnoreSpec);

dsnutilUCSLoadDecfloatOption: ( dsnutilUCSDecfloatSpec);

dsnutilUCSLoadOverrideOption: ( dsnutilUCSOverrideSpec);

dsnutilUCSLoadDrainOption: ( dsnutilUCSDrainSpec);
		...
		DSNUTIL_FORCE (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_NONE | DSNUTIL_READERS | DSNUTIL_ALL) DSNUTIL_RPAREN1
			)
		)
	);
...
dsnutilUCSLoadResumeSpec: ( dsnutilUCSLoadResumeSpecOptions);
...
dsnutilUCSLoadResumeSpecReplaceOption: ( DSNUTIL_REPLACE);
		...
		DSNUTIL_SHRLEVEL (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) dsnutilUCSLoadResumeSpecShrlevelOptions DSNUTIL_RPAREN1
			)
		)
	);
...
dsnutilUCSStatisticsSpec: (
		DSNUTIL_STATISTICS dsnutilUCSStatTableSpec? dsnutilUCSStatIndexSpec1?
...
dsnutilUCSReportOption: ( DSNUTIL_REPORT dsnutilUCSYesOrNo);
...
/*
 ...
 syntax is tolerated by
 DB2 but not mentioned in the IBM documentation as
 of this date.
 */
...
dsnutilUCSTableStatsColumnListAll: (
		DSNUTIL_COLUMN (DSNUTIL_LPAREN | DSNUTIL_LPAREN1) dsnutilUCSColumnList DSNUTIL_RPAREN1
	);

dsnutilUCSColgroupList: (
		dsnutilUCSColgroupSpec (
			DSNUTIL_COMMA dsnutilUCSColgroupSpec
		)*
	);

dsnutilUCSColgroupSpec: (
		DSNUTIL_COLGROUP (
			DSNUTIL_LPAREN dsnutilUCSColumnList DSNUTIL_RPAREN1
		) dsnutilUCSColgroupStatsSpec*
	);

dsnutilUCSUseProfile: ( DSNUTIL_USE DSNUTIL_PROFILE);

dsnutilUCSColumnList: (
		dsnutilUCSColumnName (DSNUTIL_COMMA dsnutilUCSColumnName)*
	);
		...
		DSNUTIL_TABLESAMPLE DSNUTIL_SYSTEM (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_AUTO | DSNUTIL_NONE | dsnutilUCSArg) DSNUTIL_RPAREN1
			)
...
dsnutilUCSStatsIndexNameAndStats1: (
		dsnutilUCSQualifiedIndexName dsnutilUCSCorrelationStatsSpec*
	);
...
dsnutilUCSFreqval1: (
		DSNUTIL_FREQVAL DSNUTIL_NUMCOLS dsnutilUCSArgOptionalParens (
...
/*
 ...
 Martijn Rutte 2023-10-24. This
 is tolerated by DB2 13 but is
...
dsnutilUCSFormatSpecOptions: (
		...
		| (
			(
				...
				| DSNUTIL_LPAREN1
				| DSNUTIL_DB_TS_LPAREN
			) (DSNUTIL_UNLOAD | DSNUTIL_SQLDS | DSNUTIL_INTERNAL) DSNUTIL_RPAREN1
		)
	);

dsnutilUCSSpannedOption: ( DSNUTIL_SPANNED dsnutilUCSYesOrNo);
		...
		DSNUTIL_DECFLOAT_ROUNDMODE (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) dsnutilUCSDecfloatSpecOption DSNUTIL_RPAREN1
			)
		)
	);
...
dsnutilUCSOverrideSpec: (
		DSNUTIL_OVERRIDE DSNUTIL_LPAREN dsnutilUCSOverrideSpecOption (
			DSNUTIL_COMMA dsnutilUCSOverrideSpecOption
		)* DSNUTIL_RPAREN1
	);
...
dsnutilUCSLabeledDurationExpression: (
		(
			...
			| (DSNUTIL_CURRENT_TIMESTAMP DSNUTIL_WITH_TIME_ZONE?)
		) (
		...
		DSNUTIL_SWITCHTIME (
			...
			| (
				(
					...
					dsnutilUCSTimestamp
					| dsnutilUCSLabeledDurationExpression
				) DSNUTIL_RPAREN1 dsnutilUCSNewmaxroOption?
			)
		)
	);
		...
		DSNUTIL_NEWMAXRO (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_NONE | dsnutilUCSArg) DSNUTIL_RPAREN1
			)
		)
	);
...
dsnutilUCSIntoTableSpec: (
		DSNUTIL_INTO dsnutilUCSQualifiedTableNameWithLit dsnutilUCSIntoTableSpecOptions*
	);
...
dsnutilUCSPartOption1: (DSNUTIL_PART dsnutilUCSArgOptionalParens);

dsnutilUCSPartOption2: (DSNUTIL_PART dsnutilUCSArgOptionalParens);

dsnutilUCSIntoTableSpecPreformatOption: (DSNUTIL_PREFORMAT);
...
/*
 ...
 noted "IBM Db2 z/OS is quite
 forgiving..."
 
 */
dsnutilUCSLoadFieldListOption: (
		DSNUTIL_LPAREN dsnutilUCSLoadFieldSpecification (
...
dsnutilUCSUnloadFieldListOption: (
		DSNUTIL_LPAREN dsnutilUCSUnloadFieldSpecification (
...
dsnutilUCSLoadFieldSpecification: (
		dsnutilUCSFieldNameSpec dsnutilUCSLoadFieldSpecificationType? (
			dsnutilUCSNullif
			| dsnutilUCSDefaultif
		)?
	);
...
dsnutilUCSUnloadFieldSpecification: (
		dsnutilUCSFieldNameSpec dsnutilUCSUnloadFieldSpecificationType?
	);
...
/*
 ...
 but it is in the examples. And
 I'm allowing a strip-spec because
 as long as I have to make things up, I'm going to make things
 up
...
dsnutilUCSUnloadFieldSpecificationChar: (
		(DSNUTIL_CHAR | DSNUTIL_CHARACTER) dsnutilUCSArgInParens? (
...
dsnutilUCSUnloadClobfSpec: (
		(
			...
			| DSNUTIL_BLOBF
		) dsnutilUCSTemplateName DSNUTIL_BINARYXML?
	);
...
dsnutilUCSUnloadFieldSpecificationVarchar: (
		DSNUTIL_VARCHAR dsnutilUCSArgInParens? (
...
dsnutilUCSFieldSpecificationSmallint: ( DSNUTIL_SMALLINT);

dsnutilUCSFieldSpecificationInteger: (
		(DSNUTIL_INTEGER | DSNUTIL_INT) dsnutilUCSIntegerOptions*
	);

dsnutilUCSFieldSpecificationBigint: ( DSNUTIL_BIGINT);
...
dsnutilUCSFieldSpecificationDouble: ( DSNUTIL_DOUBLE);

dsnutilUCSFieldSpecificationReal: ( DSNUTIL_REAL);

dsnutilUCSFieldSpecificationDate: (
		DSNUTIL_DATE (
			DSNUTIL_DATE_P
			| (DSNUTIL_EXTERNAL dsnutilUCSArgInParens?)
		)
	);
...
dsnutilUCSFieldSpecificationRowid: ( DSNUTIL_ROWID);

dsnutilUCSLoadFieldSpecificationBlob: ( DSNUTIL_BLOB);

dsnutilUCSUnloadFieldSpecificationBlob: (
		DSNUTIL_BLOB dsnutilUCSArgInParens? DSNUTIL_TRUNCATE?
	);
...
dsnutilUCSUnloadFieldSpecificationClob: (
		DSNUTIL_CLOB dsnutilUCSArgInParens? DSNUTIL_TRUNCATE? dsnutilUCSCCSIDPhrase?
	);
...
dsnutilUCSUnloadFieldSpecificationDbclob: (
		DSNUTIL_DBCLOB dsnutilUCSArgInParens? DSNUTIL_TRUNCATE? dsnutilUCSCCSIDPhrase?
	);
...
dsnutilUCSNullif: (
		DSNUTIL_NULLIF (
			dsnutilUCSFieldSelectionCriterion
			| (
				DSNUTIL_LPAREN1 dsnutilUCSFieldSelectionCriterion (
					DSNUTIL_RPAREN1
					| DSNUTIL_DB_TS_RPAREN
				)
			)
...
dsnutilUCSDefaultif: (
		DSNUTIL_DEFAULTIF (
			...
			| (
				DSNUTIL_LPAREN1 dsnutilUCSDefaultifCondition (
					DSNUTIL_RPAREN1
					| DSNUTIL_DB_TS_RPAREN
				)
			)
...
dsnutilUCSClobOptions: ( DSNUTIL_MIXED | dsnutilUCSCCSIDPhrase);
...
dsnutilUCSLoadDecimalSpec: (
		DSNUTIL_DECIMAL (
			DSNUTIL_PACKED
			| DSNUTIL_ZONED
			| (DSNUTIL_EXTERNAL (dsnutilUCSArgList1)?)
		)?
	);
...
dsnutilUCSDefaultifCondition: (
		DSNUTIL_CONV_ERROR
		| (
			(dsnutilUCSFieldName | dsnutilUCSArgInParens) (
				DSNUTIL_EQUAL
				| DSNUTIL_NOT_EQUAL
			) dsnutilUCSArg
		)
...
dsnutilUCSMergecopy: (
		DSNUTIL_MERGECOPY dsnutilUCSMergecopyListOrTablespace dsnutilUCSMergecopyOptions*
	);

dsnutilUCSMergecopyListOrTablespace: (
		dsnutilUCSListNameWithLit
		| (
			dsnutilUCSQualifiedTablespaceNameWithLit dsnutilUCSDsnumOption?
		)
	);
...
dsnutilUCSNewcopyOption: ( DSNUTIL_NEWCOPY dsnutilUCSYesOrNo);
...
dsnutilUCSModifyRecovery: (
		DSNUTIL_MODIFY DSNUTIL_RECOVERY dsnutilUCSModifyRecoveryListOrTablespace
...
dsnutilUCSModifyRecoveryDeleteOption: (
		DSNUTIL_DELETE (DSNUTIL_AGE | DSNUTIL_DATE) dsnutilUCSArgOptionalParens?
		...
		DSNUTIL_FLASHCOPY (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) DSNUTIL_ONLY DSNUTIL_RPAREN1
			)
		)
	);

dsnutilUCSModifyRecoveryDeletedsOption: ( DSNUTIL_DELETEDS);
...
dsnutilUCSModifyRecoveryRetainOption: (
		DSNUTIL_RETAIN (
			...
			| (
				DSNUTIL_GDGLIMIT (
					(DSNUTIL_LAST dsnutilUCSArgInParens)
					| DSNUTIL_LOGLIMIT
				)?
			)
			| (
				DSNUTIL_GDGLIMIT (
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) DSNUTIL_LOGLIMIT DSNUTIL_RPAREN1
			)
		)
	);

dsnutilUCSModifyRecoveryNocopypendOption: ( DSNUTIL_NOCOPYPEND);

dsnutilUCSModifyStatistics: (
		DSNUTIL_MODIFY DSNUTIL_STATISTICS dsnutilUCSModifyStatisticsListOrTablespaceEtAl
...
dsnutilUCSOptions: (
		DSNUTIL_OPTIONS dsnutilUCSOptionsProcessingOptionsSpec*
	);

dsnutilUCSOptionsOff: ( DSNUTIL_OFF);

dsnutilUCSOptionsKey: ( DSNUTIL_KEY dsnutilUCSArgOptionalParens);
...
dsnutilUCSOptionsPreviewOption: ( DSNUTIL_PREVIEW);
...
dsnutilUCSOptionsEventSpecOption: (
		DSNUTIL_EVENT (
			(
				DSNUTIL_LPAREN dsnutilUCSOptionsEventSpecOptionOptions (
					DSNUTIL_COMMA? dsnutilUCSOptionsEventSpecOptionOptions
				)? DSNUTIL_RPAREN1
			)
			| (
...
dsnutilUCSOptionsEventItemerrOption: (
		DSNUTIL_ITEMERROR (
			DSNUTIL_COMMA? (DSNUTIL_HALT | DSNUTIL_SKIP)
		)
	);
...
dsnutilUCSQuiesce: (
		DSNUTIL_QUIESCE dsnutilUCSQuiesceListOrTablespaceEtAl dsnutilUCSQuiesceOptions*
	);
...
dsnutilUCSQuiesceWriteOption: ( DSNUTIL_WRITE dsnutilUCSYesOrNo);

dsnutilUCSRebuildIndex: (
		DSNUTIL_REBUILD dsnutilUCSRebuildIndexSpec dsnutilUCSRebuildIndexOptions*
	);
...
dsnutilUCSRebuildIndexSpecIndex: (
		(
			DSNUTIL_INDEX DSNUTIL_DB_TS_LPAREN dsnutilUCSIndexSpec (
				DSNUTIL_COMMA dsnutilUCSIndexSpec
			)* DSNUTIL_RPAREN1
		)
		| (
...
dsnutilUCSRebuildIndexSpecIndexspace: (
		(
			DSNUTIL_INDEXSPACE DSNUTIL_DB_TS_LPAREN dsnutilUCSRebuildIndexIndexspaceSpec (
				DSNUTIL_COMMA dsnutilUCSRebuildIndexIndexspaceSpec
			)*
		) DSNUTIL_RPAREN1
		| (
...
dsnutilUCSRebuildIndexShrlevelOption: (
		DSNUTIL_SHRLEVEL (
			(
				DSNUTIL_REFERENCE
				| (
					DSNUTIL_CHANGE dsnutilUCSRebuildIndexChangeSpec*
				)
			)
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) DSNUTIL_REFERENCE DSNUTIL_RPAREN1
			)
			| (
				(
					DSNUTIL_LPAREN
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) DSNUTIL_CHANGE DSNUTIL_RPAREN1 dsnutilUCSRebuildIndexChangeSpec*
			)
		)
	);
...
dsnutilUCSRebuildIndexStatsSpec: (
		DSNUTIL_STATISTICS dsnutilUCSRebuildIndexStatsSpecOptions*
	);
		...
		DSNUTIL_RECOVER (
			(
				(
					dsnutilUCSListNameWithLit
					| (
						dsnutilUCSRecoverObjectSpec dsnutilUCSDsnumOption?
					)+
			...
			| (
				dsnutilUCSRecoverObjectSpec DSNUTIL_PAGE dsnutilUCSArgOptionalParens
					DSNUTIL_CONTINUE?
			)
		) dsnutilUCSRecoverSiteOption? dsnutilUCSLograngesOption?
	);

dsnutilUCSLograngesOption: (DSNUTIL_LOGRANGES dsnutilUCSYesOrNo);
		...
		DSNUTIL_SCOPE (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_UPDATED | DSNUTIL_ALL) DSNUTIL_RPAREN1
			)
		)
	);
...
dsnutilUCSRecoverOptionsSpec: (
		dsnutilUCSRecoverObjectSpec dsnutilUCSDsnumOption? (
			(
				DSNUTIL_TOCOPY dsnutilUCSDsn dsnutilUCSRecoverImageCopySpec?
...
dsnutilUCSVerifysetOption: (DSNUTIL_VERIFYSET dsnutilUCSYesOrNo);

dsnutilUCSEnforceOption: ( DSNUTIL_ENFORCE dsnutilUCSYesOrNo);

dsnutilUCSTorbaOption: (
		(DSNUTIL_TORBA | DSNUTIL_TOLOGPOINT) dsnutilUCSArgOptionalParens
	);

dsnutilUCSLogonlyOption: ( DSNUTIL_LOGONLY);
...
dsnutilUCSCurrentCopyOnlyOption: ( DSNUTIL_CURRENTCOPYONLY);
...
dsnutilUCSRecoverFromSpecTablespaceFrom: (
		DSNUTIL_FROM dsnutilUCSQualifiedTablespaceName dsnutilUCSDsnumOption?
	);

dsnutilUCSRecoverFromSpecIndexspace: (
		dsnutilUCSQualifiedIndexspaceNameWithLit dsnutilUCSDsnumOption?
			dsnutilUCSRecoverFromSpecIndexspaceFrom?
	);

dsnutilUCSRecoverFromSpecIndexspaceFrom: (
		DSNUTIL_FROM dsnutilUCSQualifiedIndexspaceName dsnutilUCSDsnumOption?
	);

dsnutilUCSRecoverFromSpecIndex: (
		dsnutilUCSQualifiedIndexNameWithLit dsnutilUCSDsnumOption?
			dsnutilUCSRecoverFromSpecIndexFrom?
	);

dsnutilUCSRecoverFromSpecIndexFrom: (
		DSNUTIL_FROM dsnutilUCSQualifiedIndexName dsnutilUCSDsnumOption?
	);
...
dsnutilUCSRecoverCurrentcopyonlyOption: (DSNUTIL_CURRENTCOPYONLY);
		...
		DSNUTIL_NOSYSCOPY (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_INLCOPY | DSNUTIL_FCCOPY) DSNUTIL_RPAREN1
			)
		)?
	);
		...
		DSNUTIL_FLASHCOPY_PPRCP (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) dsnutilUCSFlashcopyPprcpOptionOptions DSNUTIL_RPAREN1
			)
		)
	);
...
dsnutilUCSRecoverImageCopySpec: (
		DSNUTIL_TOVOLUME (
			...
			| (
				dsnutilUCSVolSer (
					DSNUTIL_TOSEQNO dsnutilUCSVolSeqNo
				)?
			)
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) dsnutilUCSVolSer DSNUTIL_RPAREN1 (
					DSNUTIL_TOSEQNO dsnutilUCSVolSeqNo
				)?
			)
			| (
				dsnutilUCSVolSer DSNUTIL_TOSEQNO (
					DSNUTIL_LPAREN
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) dsnutilUCSVolSeqNo DSNUTIL_RPAREN1
			)
			| (
				(
					DSNUTIL_LPAREN
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) dsnutilUCSVolSer DSNUTIL_RPAREN1 DSNUTIL_TOSEQNO (
					DSNUTIL_LPAREN
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) dsnutilUCSVolSeqNo DSNUTIL_RPAREN1
		...
		DSNUTIL_SHRLEVEL (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) DSNUTIL_CHANGE DSNUTIL_RPAREN1 (
					dsnutilUCSDeadlineOption
					| dsnutilUCSReorgIndexDrainSpec
					| dsnutilUCSChangeSpec
				)*
...
dsnutilUCSReorgIndexStatsSpec: (
		DSNUTIL_STATISTICS dsnutilUCSReorgIndexStatsSpecOptions+
	);
		...
		DSNUTIL_TIMEOUT (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_TERM | DSNUTIL_ABEND) DSNUTIL_RPAREN1
			)
		)
	);
...
dsnutilUCSReorgIndexNosysut1Option: ( DSNUTIL_NOSYSUT1);
...
dsnutilUCSLastlogOption: ( DSNUTIL_LASTLOG dsnutilUCSYesOrNo);
		...
		DSNUTIL_DRAIN (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_ALL | DSNUTIL_WRITERS) DSNUTIL_RPAREN1
			)
		)
	);
		...
		DSNUTIL_REORG (
			(
				DSNUTIL_TABLESPACE_LIST dsnutilUCSListName (
					DSNUTIL_LISTPARTS dsnutilUCSArgOptionalParens
				)?
			)
			| (
				dsnutilUCSQualifiedTablespaceNameWithLit (
					DSNUTIL_PART dsnutilUCSArgList1
				)?
			)
...
/*
 ...
 but ignored has
 implications for other options. For example, dsnutilUCSChangeSpec
 is only part of two other
 (SHRLEVEL) options. Because we may see the
 SORTKEYS token interspersed with other tokens
 ...
 included here on their
 own.
 
 It is also possible that the syntax diagram is simply not communicating its
 intent to me,
 and options which appear to be embedded in other options are
 in fact independent.
 */
dsnutilUCSReorgTablespaceOptions: (
		dsnutilUCSCloneOption
 ...
 */
dsnutilUCSReorgTablespaceSortkeysOption: ( DSNUTIL_SORTKEYS);
...
/*
 ...
 2023-10-24. This is tolerated by DB2
 13 but is not documented.
 */
dsnutilUCSUnlddnOption: (
...
dsnutilUCSKeepdictionaryOption: ( DSNUTIL_KEEPDICTIONARY);
...
dsnutilUCSAuxOption: ( DSNUTIL_AUX dsnutilUCSYesOrNo);

dsnutilUCSAutoestspaceOption: (
		DSNUTIL_AUTOESTSPACE dsnutilUCSYesOrNo
	);

dsnutilUCSNosysrecOption: ( DSNUTIL_NOSYSREC);

/*
 This is simpler than the previous incarnation of this rule,
 and although it allows syntax that the
 syntax diagram
...
dsnutilUCSSortdataOption: (
		DSNUTIL_SORTDATA dsnutilUCSYesOrNo? (
...
dsnutilUCSRebalanceOption: (
		DSNUTIL_REBALANCE (DSNUTIL_SORTCLUSTER dsnutilUCSYesOrNo)?
	);
...
dsnutilUCSShrlevelChangeSpec2: (
		...
		| (
			DSNUTIL_SHRLEVEL (
				...
				| DSNUTIL_LPAREN1
				| DSNUTIL_DB_TS_LPAREN
			) DSNUTIL_CHANGE DSNUTIL_RPAREN1 DSNUTIL_REGISTER DSNUTIL_YES
		)
		| (
			DSNUTIL_SHRLEVEL DSNUTIL_CHANGE DSNUTIL_REGISTER (
				DSNUTIL_LPAREN
				| DSNUTIL_LPAREN1
				| DSNUTIL_DB_TS_LPAREN
			) DSNUTIL_YES DSNUTIL_RPAREN1
		)
		| (
			DSNUTIL_SHRLEVEL (
				DSNUTIL_LPAREN
				| DSNUTIL_LPAREN1
				| DSNUTIL_DB_TS_LPAREN
			) DSNUTIL_CHANGE DSNUTIL_RPAREN1 DSNUTIL_REGISTER (
				DSNUTIL_LPAREN
				| DSNUTIL_LPAREN1
				| DSNUTIL_DB_TS_LPAREN
			) DSNUTIL_YES DSNUTIL_RPAREN1
...
/*
 ...
 to dsnutilUCSIsolationCSOption
 for clarity per Martijn Rutte 2023-11-03.
 
 Made dsnutilUCSRegisterOption optional per Martijn
 Rutte 2023-04-03.
 */
		...
		DSNUTIL_ISOLATION (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) DSNUTIL_UR DSNUTIL_RPAREN1
			)
		)
	);

dsnutilUCSRegisterOption: ( DSNUTIL_REGISTER dsnutilUCSYesOrNo);
			...
			DSNUTIL_MAPPINGTABLE (
				...
				| (
					(
						...
						| DSNUTIL_LPAREN1
						| DSNUTIL_DB_TS_LPAREN
					) dsnutilUCSQualifiedTableName DSNUTIL_RPAREN1
				)
			)
		)
		| (
			DSNUTIL_MAPPINGDATABASE (
				...
				| (
					(
						...
						| DSNUTIL_LPAREN1
						| DSNUTIL_DB_TS_LPAREN
					) dsnutilUCSDatabaseName DSNUTIL_RPAREN1
				)
			)
		)
	);
		...
		DSNUTIL_UNLOAD (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) DSNUTIL_EXTERNAL DSNUTIL_RPAREN1
			)
...
dsnutilUCSFromTableSpec1: (
		DSNUTIL_FROM_TABLE dsnutilUCSQualifiedTableName dsnutilUCSWhenCondition?
	);

dsnutilUCSWhenCondition: (
		DSNUTIL_WHEN DSNUTIL_WHEN_LPAREN NOT? predicate (
			(AND | OR) NOT? LPAREN* predicate RPAREN*
		)* RPAREN
	);

dsnutilUCSFromTableSpec2: (
		DSNUTIL_FROM_TABLE dsnutilUCSQualifiedTableName dsnutilUCSFromTableSpec2Options*
			dsnutilUCSWhenCondition?
	);
...
dsnutilUCSNopadOption1: ( DSNUTIL_NOPAD dsnutilUCSYesOrNo);

dsnutilUCSNopadOption2: ( DSNUTIL_NOPAD);
...
dsnutilUCSFieldName: dsnutilUCSArg;

dsnutilUCSListName: dsnutilUCSArg;

dsnutilUCSListNameWithLit: (DSNUTIL_LIST dsnutilUCSListName);

dsnutilUCSVolSer: dsnutilUCSArg;

dsnutilUCSVolSeqNo: dsnutilUCSArg;

dsnutilUCSRepair: (
		DSNUTIL_REPAIR DSNUTIL_OBJECT? (
			DSNUTIL_LOG dsnutilUCSYesOrNo
		)? dsnutilUCSRepairOptions+ dsnutilUCSCloneOption?
	);
		...
		DSNUTIL_RID (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) DSNUTIL_HEX_LIT DSNUTIL_RPAREN1
			)
		)
	);
...
dsnutilUCSPageOption: (DSNUTIL_PAGE dsnutilUCSArgOptionalParens);
...
dsnutilUCSLobTablespaceSpec: (
		DSNUTIL_TABLESPACE dsnutilUCSQualifiedXmlTablespacename dsnutilUCSRowidOption
...
dsnutilUCSXmlTablespaceSpec: (
		DSNUTIL_TABLESPACE dsnutilUCSQualifiedXmlTablespacename dsnutilUCSDocidOption
			dsnutilUCSRepairDeleteStatement
	);
...
dsnutilUCSVerifyStatement: (
		DSNUTIL_VERIFY dsnutilUCSRepairOffsetOption? dsnutilUCSRepairDataOption+
	);
		...
		DSNUTIL_DELETE (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) DSNUTIL_DATAONLY DSNUTIL_RPAREN1
			)
		)?
	);

dsnutilUCSDumpStatement: (
		DSNUTIL_DUMP dsnutilUCSDumpStatementOptions+
	);
...
dsnutilUCSRepairSetTablespace: (
		dsnutilUCSQualifiedTablespaceNameWithLit dsnutilUCSPartOption2?
...
dsnutilUCSRepairSetIndex: (
		DSNUTIL_INDEX (
			(
				DSNUTIL_DB_TS_LPAREN dsnutilUCSQualifiedIndexName dsnutilUCSPartOption2?
					DSNUTIL_RPAREN1
			)
			| (
...
dsnutilUCSRepairSetIndexspace: (
		DSNUTIL_INDEXSPACE (
			(
				DSNUTIL_DB_TS_LPAREN dsnutilUCSQualifiedIndexspaceName dsnutilUCSPartOption2?
					DSNUTIL_RPAREN1
			)
			| (
				dsnutilUCSAllInParens dsnutilUCSQualifiedTablespaceNameWithLit
...
dsnutilUCSDbdStatement: (
		DSNUTIL_DBD (
			(
				DSNUTIL_DROP DSNUTIL_DATABASE dsnutilUCSDatabaseName DSNUTIL_DBID
					dsnutilUCSArgOptionalParens
			)
			| (
				(
					...
					DSNUTIL_OUTDDN dsnutilUCSArgOptionalParens
				)?
			)
		)
	);
...
dsnutilUCSSystempagesStatement: (
		DSNUTIL_INSERTVERSIONPAGES DSNUTIL_SETCURRENTVERSION?
			dsnutilUCSQualifiedTablespaceNameWithLit (
			(DSNUTIL_SHRLEVEL DSNUTIL_CHANGE)
			| (
				DSNUTIL_SHRLEVEL (
...
dsnutilUCSWritelogStatement: (
		DSNUTIL_WRITELOG dsnutilUCSQualifiedTablespaceNameWithLit dsnutilUCSWritelogStatementOptions
			+
	);
...
dsnutilUCSReportTablespaceListOption: (
		dsnutilUCSReportTablespaceListSpec (
			DSNUTIL_INDEX dsnutilUCSArgOptionalParens
		)?
	);
...
dsnutilUCSCurrentOption: ( DSNUTIL_CURRENT);

dsnutilUCSSummaryOption: ( DSNUTIL_SUMMARY);

dsnutilUCSLocalsiteOption: ( DSNUTIL_LOCALSITE);

dsnutilUCSRecoverysiteOption: ( DSNUTIL_RECOVERYSITE);
...
dsnutilUCSReportLogonlySpec: (
		DSNUTIL_LOGONLY dsnutilUCSSwitchVcatOption?
	);
...
/*
 ...
 ANTLR has a problem (as do I)
 with coding something with no required
 parameters. So I code it this way, then make them all
 optional with
...
dsnutilUCSReportFromdumpSpec: (
		DSNUTIL_FROMDUMP dsnutilUCSReportFromdumpOptions*
	);
...
dsnutilUCSRunstatsIndexListSpec: (
		DSNUTIL_DB_TS_LPAREN dsnutilUCSRunstatsIndexAndSpecs (
...
/*
 ...
 documented, but DB2 tolerates it.
		...
		DSNUTIL_DELETE (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) DSNUTIL_PROFILE DSNUTIL_RPAREN1
			)
		)
	);
 ...
 */
dsnutilUCSStogroupName: dsnutilUCSArg;
...
dsnutilUCSDsnExpression: ( dsnutilUCSDsnOptions+);
...
dsnutilUCSDsnWithLit: ( DSNUTIL_DSN dsnutilUCSDsn);
...
dsnutilUCSDsnSubsysSpec: (
		dsnutilUCSDsnSubsysOption dsnutilUCSDsnLreclOption dsnutilUCSDsnRecfmOption
	);
...
dsnutilUCSDsnModelDcbOption: (DSNUTIL_MODELDCB dsnutilUCSDsn);
		...
		DSNUTIL_BLKSZLIM DSNUTIL_EQUAL? (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (dsnutilUCSArg DSNUTIL_BLKSZLIM_SUFFIX?) DSNUTIL_RPAREN1
			)
		)
	);
...
dsnutilUCSDsnEattrOption: DSNUTIL_EATTR;
...
dsnutilUCSTemplateName: dsnutilUCSArg;

dsnutilUCSPathName: dsnutilUCSArg;
...
/*
 ...
 options are tolerated out of the
...
dsnutilUCSUnload: (
		(
			(
				(DSNUTIL_UNLOAD_DATA | DSNUTIL_UNLOAD) (
					dsnutilUCSUnloadSpec* dsnutilUCSFromTableSpec2
				)+
			)
			| (
				DSNUTIL_UNLOAD dsnutilUCSUnloadSourceSpec (
					dsnutilUCSUnloadSpec* dsnutilUCSFromTableSpec2
				)*
			)
...
dsnutilUCSUnloadSourceSpec: (
		(DSNUTIL_FROM_TABLESPACE | DSNUTIL_TABLESPACE) dsnutilUCSQualifiedTablespaceName (
		...
		DSNUTIL_FLOAT (
			...
			| (
				(
					...
					| DSNUTIL_LPAREN1
					| DSNUTIL_DB_TS_LPAREN
				) (DSNUTIL_S390 | DSNUTIL_IEEE) DSNUTIL_RPAREN1
			)
		)
	);
...
dsnutilUCSDdname: dsnutilUCSArg;
...
dsnutilUCSDatabaseName: dsnutilUCSDatabaseObjectName;

dsnutilUCSTablespaceName: dsnutilUCSDatabaseObjectName;

dsnutilUCSTableName: dsnutilUCSDatabaseObjectName;

dsnutilUCSSchemaName: dsnutilUCSDatabaseObjectName;

dsnutilUCSColumnName: dsnutilUCSDatabaseObjectName;

dsnutilUCSCreatorID: dsnutilUCSDatabaseObjectName;

dsnutilUCSIndexName: dsnutilUCSDatabaseObjectName;

dsnutilUCSIndexspaceName: dsnutilUCSDatabaseObjectName;

dsnutilUCSXmlTablespacename: dsnutilUCSDatabaseObjectName;
...
/*
 ...
 other keywords must be added here.
 For example, someone might
 use WORKDDN as the ddname for the WORKDDN keyword. Or maybe
 someone
 wants the list they define with LISTDEF to be named LIST.
 
 Bear in mind that all these keywords
 come out of the "embedded"
 lexer for the SYSPROC.DSNUTILx stored procedure's third parameter,
 so
 "normal" DB2z SQL keywords need not be listed here.
 */
dsnutilUCSKeyword: (
		DSNUTIL_ABEND
		| DSNUTIL_ACCESSPATH
...
dsnutilUCSArgList1: (
		(DSNUTIL_LPAREN | DSNUTIL_LPAREN1 | DSNUTIL_DB_TS_LPAREN) dsnutilUCSArg (
			DSNUTIL_COMMA dsnutilUCSArg
		)* DSNUTIL_RPAREN1
	);

dsnutilUCSArgList2: (
		(
			(DSNUTIL_LPAREN | DSNUTIL_LPAREN1) dsnutilUCSArg (
				DSNUTIL_COMMA dsnutilUCSArg
			)* DSNUTIL_RPAREN1
		)
		| (
 ...
 */
dsnutilUCSArgList3: (
		(DSNUTIL_LPAREN | DSNUTIL_LPAREN1 | DSNUTIL_DB_TS_LPAREN) dsnutilUCSArg dsnutilUCSArg? (
			DSNUTIL_COMMA dsnutilUCSArg
		)* DSNUTIL_RPAREN1
	);
...
callArgumentList: (callArgument (COMMA callArgument)*);
...
createAuxiliaryTableStatement: (
		CREATE (AUX | AUXILIARY) TABLE auxTableName IN (
			databaseName DOT
		)? tablespaceName STORES tableName appendClause? COLUMN columnName PART INTEGERLITERAL
	);
...
createFunctionStatementExternalScalar: (
		CREATE FUNCTION functionName LPAREN (
			parameterDeclaration1 (COMMA parameterDeclaration1)*
		)? RPAREN createFunctionStatementExternalScalarReturnsPhrase
...
createFunctionStatementExternalScalarReturnsPhrase: (
		RETURNS (
			...
			| (
				functionDataType CAST FROM functionDataType (
					AS LOCATOR
				)?
			)
		)
...
createFunctionStatementExternalTable: (
		CREATE FUNCTION functionName LPAREN (
			parameterDeclaration1 (COMMA parameterDeclaration1)*
		)? RPAREN createFunctionStatementExternalTableReturnsPhrase
...
createFunctionStatementExternalTableReturnsPhrase: (
		RETURNS (
			(
				TABLE LPAREN externalTableFunctionColumn (
					COMMA externalTableFunctionColumn
				)* RPAREN
			)
			| (GENERIC TABLE)
		)
...
createFunctionStatementSourced: (
		CREATE FUNCTION functionName LPAREN (
			parameterDeclaration1 (COMMA parameterDeclaration1)*
		)? RPAREN createFunctionStatementSourcedReturnsPhrase createFunctionStatementSourcedOptions*
...
createFunctionStatementSourcedSourcePhrase: (
		SOURCE (
			(
				functionName LPAREN parameterDeclaration1 (
...
createFunctionStatementInlineSqlScalar: (
		CREATE FUNCTION functionName LPAREN (
			...
			(WRAPPED obfuscatedStatementText)
...
createGlobalTemporaryTableStatement: (
		CREATE GLOBAL TEMPORARY TABLE tableName (
			(
				LPAREN createGlobalTemporaryTableColumnDefinition (
					COMMA createGlobalTemporaryTableColumnDefinition
				)* RPAREN
			)
			| (LIKE tableName)
		) ccsidClause1?
 ...
 */
createIndexStatement: (
		CREATE (TYPE (INTEGERLITERAL))? (
			UNIQUE (WHERE NOT NULL)?
		)? INDEX indexName ON (
			(
				tableName LPAREN (columnName | keyExpression) (
					ASC
					| DESC
					| RANDOM
				)? (
					COMMA (columnName | keyExpression) (
						ASC
						| DESC
						| RANDOM
					)?
...
createMaskStatement: (
		CREATE MASK maskName ON tableName (AS? correlationName)? FOR COLUMN columnName RETURN
			caseExpression enableDisableOption?
	);

createPermissionStatement: (
		CREATE PERMISSION permissionName ON tableName (
			AS? correlationName
...
/*
 ...
 Reversing the order makes WRAPPED 
...
sqlRoutineBody: probablySQLPL+;

obfuscatedStatementText: (identifier | COLON | UNIDENTIFIED)+;

probablySQLPL: (sqlplProcedureStatement SEMICOLON*);
...
createSequenceStatement: (
		CREATE SEQUENCE sequenceName createSequenceOptionList (
			COMMA? createSequenceOptionList
		)*
	);
...
createStogroupStatement: (
		...
		)? VCAT catalogName dataclasOption? mgmtclasOption? storclasOption? keyLabelOption?
	);

/*
 Moved options to createTableOptions rule so they could
 be in any order. Suggested by Martijn Rutte
 2023-01-10.
 */
createTableStatement: (
...
createTablespaceStatement: (
		CREATE TABLESPACE tablespaceName createTablespaceOptionList*
	);

/*
 The syntax accepts WRAPPED obfuscatedStatementText, but not
 in a static SQL context so it is not
 supported here.
 */
createTriggerStatement: (
...
createTrustedContextStatement: (
		CREATE TRUSTED CONTEXT contextName BASED UPON CONNECTION USING SYSTEM AUTHID
			authorizationName (
...
createTypeArrayStatement: (
		CREATE TYPE arrayTypeName AS createTypeArrayBuiltinType ARRAY OPENSQBRACKET (
			INTEGERLITERAL
...
createTypeDistinctStatement: (
		CREATE TYPE distinctTypeName AS sourceDataType (
...
/*
 ...
 ...has been made optional, as it should
...
createViewStatement: (
		...
		)? fullSelect createViewCheckOptionClause?
	);
...
declareCursorStatement: (
		DECLARE cursorName (
			...
			| returnability
			| rowsetPositioning
		)* FOR (selectStatement | statementName)
	);
		...
		DECLARE GLOBAL TEMPORARY TABLE tableName (
			(
				LPAREN declareGlobalTemporaryTableColumnDefinition (
					COMMA declareGlobalTemporaryTableColumnDefinition
				)* RPAREN
			)
...
declareTableStatement: (
		...
		) (COMMA columnName dataType notNullPhrase?)* RPAREN
	);
...
deleteStatement: (searchedDelete | positionedDelete);
...
endDeclareSectionStatement: (END DECLARE SECTION);
...
executeStatement: (
		EXECUTE statementName (
			(
				USING (variable | arrayElementSpecification) (
					COMMA (variable | arrayElementSpecification)
				)*
			)
...
/*
 ...
 token corresponds to what the
 documentation refers to as 
 string-expression, which "is any PL/I expression that yields 
 a
 string." I'm not going to implement a grammar for the 
...
/*
 ...
 as required. It is optional here
 because one of its variants is 
 optional.
 */
fetchStatement: (
		FETCH (INSENSITIVE | SENSITIVE)? (WITH CONTINUE)? fetchOrientation? FROM? cursorName (
			singleRowFetch
			| multipleRowFetch
		)?
	);
...
includeStatement: (INCLUDE memberName);

insertStatement: (
		INSERT INTO tableName (
			LPAREN columnName (COMMA columnName)* RPAREN
		)? includeColumns? (OVERRIDING USER VALUE)? (
			(
				VALUES (
					(valuesList1 (COMMA valuesList1)*)
					| insertStatementListOfValues (
						COMMA insertStatementListOfValues
					)*
				)
			)
			| (
...
labelStatement: (
		LABEL ON (
			...
			| (
				tableName LPAREN columnName IS NONNUMERICLITERAL (
...
lockTableStatement: (
		LOCK TABLE tableName ((PARTITION | PART) INTEGERLITERAL)? IN (
			SHARE
			| EXCLUSIVE
		) MODE_
	);
...
mergeStatement: (
		MERGE INTO tableName correlationClause? includeColumns? USING (
			(LPAREN* tableReference RPAREN*)
			| sourceValues
		) ON searchCondition (
			WHEN matchingCondition THEN (
				modificationOperation
				| signalStatement
			)
		)+ (ELSE IGNORE)? notAtomicPhrase? querynoClause?
	);
...
/*
 ...
 not implemented because it is specific
 to PL/I, string-expression
...
releaseSavepointStatement: (RELEASE TO? SAVEPOINT savepointName);
...
setAssignmentStatement: ( SET setAssignmentClause);
...
/*
 ...
 conform to the pattern set by all
 the others. There's always one.
 */
setEncryptionPasswordStatement: (
		SET ENCRYPTION_PASSWORD EQ? (
			NONNUMERICLITERAL
			| variable
...
/*
 ...
 is a synonym for the latter. And
 sometimes not.
 */
setPathStatement: (
		SET (PATH | CURRENT_PATH) EQ? expression (
			COMMA? expression
		)*
	);
...
/*
 ...
 Sometimes the former is a synonym for either of
...
/*
 ...
 and TIMEZONE are all synonyms for
 each other in this context.
 Changing the lexer rule for SESSION_TIME_ZONE to match all of
 the
 above broke other things that expect TIME and ZONE to be
 separate tokens. So here we are.
 */
setSessionTimezoneStatement: (
		SET (SESSION_TIME_ZONE | (TIME ZONE) | TIMEZONE) EQ? (
			variable
			| NONNUMERICLITERAL
		)
	);
...
updateStatement: (searchedUpdate | positionedUpdate);
		...
		VALUES (
			...
			(
				valuesIntoTargetVariable (
					COMMA valuesIntoTargetVariable
				)*
			)
...
wheneverStatement: (
		WHENEVER ((NOT FOUND) | SQLERROR | SQLWARNING) (
			CONTINUE
			| ((GOTO | (GO TO)) COLON? hostLabel)
		)
	);

/*
 These happen to be the same right now. Taking advantage of that,
 but insulating myself against
 future changes too.
 */
valuesIntoTargetVariable: (setAssignmentTargetVariable);
...
grantCollectionStatement: (
		GRANT (CREATE | PACKADM) (ON | IN) COLLECTION (
			(collectionID (COMMA collectionID)*)
			| SPLAT
		) TO grantee (COMMA grantee)* withGrantOption?
	);

grantDatabaseStatement: (
		GRANT grantDatabaseAuthority (
			COMMA grantDatabaseAuthority
		)* ON DATABASE databaseName (COMMA databaseName)* TO grantee (
			COMMA grantee
		)* withGrantOption?
	);
		...
		GRANT (
			...
			| (
				grantPackageAuthority (
					COMMA grantPackageAuthority
				)*
			)
		) ON PACKAGE packageSpecification (
			COMMA packageSpecification
		)* TO grantee (COMMA grantee)* withGrantOption?
	);

grantPlanStatement: (
		GRANT grantPlanAuthority (COMMA grantPlanAuthority)* ON PLAN planName (
			COMMA planName
		)* TO grantee (COMMA grantee)* withGrantOption?
	);

grantSchemaStatement: (
		GRANT grantSchemaAuthority (COMMA grantSchemaAuthority)* ON SCHEMA (
			SPLAT
...
grantSequenceStatement: (
		GRANT grantSequenceAuthority (
			COMMA grantSequenceAuthority
		)* ON SEQUENCE sequenceName (COMMA sequenceName)* TO grantee (
			COMMA grantee
		)* withGrantOption?
	);
 ...
 */
grantTableStatement: (
		GRANT grantTableAuthority (COMMA grantTableAuthority)* ON TABLE? tableName (
			COMMA tableName
		)* TO grantee (COMMA grantee)* withGrantOption?
	);
...
grantVariableStatement: (
		GRANT grantVariableAuthority (
			COMMA grantVariableAuthority
		)* ON VARIABLE variableName (COMMA variableName)* TO grantee (
			COMMA grantee
		)* withGrantOption?
	);

grantUseOfStatement: (
		GRANT USE OF grantUseOfTarget TO grantee (COMMA grantee)* withGrantOption?
	);
...
revokeCollectionStatement: (
		REVOKE (CREATE | PACKADM) (ON | IN) COLLECTION (
			(collectionID (COMMA collectionID)*)
			| SPLAT
		) FROM grantee (COMMA grantee)* revokeByOption? revokeDependentPrivilegesOption?
	);

revokeDatabaseStatement: (
		REVOKE grantDatabaseAuthority (
			COMMA grantDatabaseAuthority
		)* ON DATABASE databaseName (COMMA databaseName)* FROM grantee (
			COMMA grantee
		)* revokeByOption? revokeDependentPrivilegesOption?
	);
...
revokeFunctionOrProcedureStatement: (
		...
		) FROM grantee (COMMA grantee)* revokeByOption? revokeDependentPrivilegesOption? RESTRICT?
	);
...
revokePlanStatement: (
		REVOKE grantPlanAuthority (COMMA grantPlanAuthority)* ON PLAN planName (
			COMMA planName
		)* FROM grantee (COMMA grantee)* revokeByOption? revokeDependentPrivilegesOption?
	);

revokeSchemaStatement: (
		REVOKE grantSchemaAuthority (COMMA grantSchemaAuthority)* ON SCHEMA (
			SPLAT
			| (schemaName (COMMA schemaName)*)
		) FROM grantee (COMMA grantee)* revokeByOption? revokeDependentPrivilegesOption?
	);
...
revokeSequenceStatement: (
		REVOKE grantSequenceAuthority (
			COMMA grantSequenceAuthority
		)* ON SEQUENCE sequenceName (COMMA sequenceName)* FROM grantee (
			COMMA grantee
		)* revokeByOption? revokeDependentPrivilegesOption? RESTRICT?
	);
...
revokeTableStatement: (
		REVOKE grantTableAuthority (COMMA grantTableAuthority)* ON TABLE? tableName FROM grantee (
			COMMA grantee
		)* revokeByOption? revokeDependentPrivilegesOption?
	);
...
/*
 ...
 comes before the
 revokeDependentPrivilegesOption in this
 particular case.
 */
revokeVariableStatement: (
		REVOKE grantVariableAuthority (
			COMMA grantVariableAuthority
		)* ON VARIABLE variableName (COMMA variableName)* FROM grantee (
			COMMA grantee
		)* revokeByOption? RESTRICT? revokeDependentPrivilegesOption?
	);

revokeUseOfStatement: (
		REVOKE USE OF grantUseOfTarget FROM grantee (
			COMMA grantee
		)* revokeByOption? revokeDependentPrivilegesOption?
	);
...
/*
 ...
 SQL "reserved words" to be used as
 labels.
 Noted by Martijn Rutte 2023-06-05.
 */
sqlplStartLabel: identifier2 COLON;
...
sqlplCaseStatement: (
		...
		) sqlplCaseElseClause? END_CASE
	);

sqlplCompoundStatement: (
		sqlplStartLabel? BEGIN sqlplCompoundAtomicClause? sqlplCompoundInitialDeclarations*
			sqlplStatementDeclaration* sqlplDeclareCursorStatement* sqlplHandlerDeclaration* (
			sqlplProcedureStatement SEMICOLON?
		)* END sqlplEndLabel? SEMICOLON?
	);

sqlplForStatement: (
		sqlplStartLabel? FOR (forLoopName AS)? (
			cursorName CURSOR holdability? FOR
		)? selectStatement DO (sqlplProcedureStatement SEMICOLON)+ END_FOR sqlplEndLabel?
	);
...
sqlplIfStatement: (
		sqlplStartLabel? IF searchCondition THEN (
			sqlplProcedureStatement SEMICOLON
		)+ sqlplIfElseIfPhrase* sqlplIfElsePhrase? END_IF SEMICOLON?
	);

sqlplIfElseIfPhrase: (
		ELSEIF searchCondition THEN (
			sqlplProcedureStatement SEMICOLON
		)+
	);

sqlplIfElsePhrase: (ELSE (sqlplProcedureStatement SEMICOLON)+);
...
sqlplLoopStatement: (
		sqlplStartLabel? LOOP (sqlplProcedureStatement SEMICOLON)+ END_LOOP sqlplEndLabel? SEMICOLON
			?
	);

sqlplRepeatStatement: (
		sqlplStartLabel? REPEAT (
			sqlplProcedureStatement SEMICOLON
		)+ UNTIL searchCondition END_REPEAT sqlplEndLabel? SEMICOLON?
	);

/*
 RESIGNAL on its own is legal per
 https://www.ibm.com/docs/en/db2-for-zos/13?topic=cnsp-raising-condition-within-sql-procedure-by-using-signal-resignal-statements
...
sqlplReturnStatement: (
		sqlplStartLabel? RETURN (expression | NULL | fullSelect)? SEMICOLON?
	);

sqlplSignalStatement: (
		sqlplStartLabel? SIGNAL sqlplSignalValue sqlplSignalInformation? SEMICOLON?
	);

sqlplWhileStatement: (
		sqlplStartLabel? WHILE searchCondition DO (
			sqlplProcedureStatement SEMICOLON
		)+ END_WHILE sqlplEndLabel? SEMICOLON?
	);

forLoopName: identifier;

/*
 The documentation says the second part of the conditional should read...
 
 (SQLSTATE VALUE?
 (literal | sqlVariableName | sqlParameterName))
 
 ...but the definition of sqlVariableName and
 sqlParameterName are such
 that there is no way to distinguish between them in this context.
 */
sqlplSignalValue: (
		sqlConditionName
		| (SQLSTATE VALUE? (literal | sqlVariableName))
	);

/*
 MESSAGE_TEXT can be set to a literal or a sqlVariableName, the latter was
 noted by Martijn Rutte
 2023-06-05.
 */
sqlplSignalInformation: (
		(SET MESSAGE_TEXT EQ (literal | sqlVariableName))
		| (LPAREN literal RPAREN)
	);

sqlplResignalValue: sqlplSignalValue;

sqlplResignalInformation: (SET MESSAGE_TEXT EQ literal);
...
/*
 ...
 ...as Martijn Rutte noticed on 01-Feb-2024 that the
 syntactically
 valid..
 
 DECLARE W_S_Q_L_I_D CHAR(128) CCSID EBCDIC FOR SBCS DATA;
 DECLARE
 W_SQLTEXT CHAR(100) CCSID EBCDIC FOR SBCS DATA DEFAULT '1' ;
...
sqlplReturnCodesDeclaration: (
		DECLARE (
			(
				SQLSTATE (
					(CHAR | CHARACTER) LPAREN INTEGERLITERAL RPAREN DEFAULT literal
				)
			)
...
sqlplDeclareCursorStatement: (declareCursorStatement SEMICOLON);
...
sqlplHandlerSpecificConditionList: (
		sqlplHandlerSpecificCondition (
			COMMA sqlplHandlerSpecificCondition
		)*
	);

sqlplHandlerSpecificCondition: (
		(SQLSTATE VALUE? literal)
		| sqlConditionName
	);

sqlplHandlerGeneralConditionList: (
		sqlplHandlerGeneralCondition (
			COMMA sqlplHandlerGeneralCondition
		)*
	);
...
sqlplCompoundAtomicClause: (NOT? ATOMIC);
...
sqlplCaseElseClause: (ELSE (sqlplProcedureStatement SEMICOLON)+);
...
grantUseOfTarget: (
		...
		| (
			TABLESPACE (databaseName DOT)? tablespaceName (
				COMMA (databaseName DOT)? tablespaceName
			)*
		)
	);

grantVariableAuthority: ( (ALL PRIVILEGES?) | READ | WRITE);
...
grantSystemAuthority: (
		...
		| DATAACCESS
		| (
			DBADM ((WITH | WITHOUT) ACCESSCTRL)? (
				(WITH | WITHOUT) DATAACCESS
			)?
		)
		| DEBUGSESSION
		| DISPLAY
...
grantSequenceAuthority: ( ALTER | USAGE);

grantSchemaAuthority: ( ALTERIN | CREATEIN | DROPIN);

grantPlanAuthority: ( BIND | EXECUTE);

grantPackageAuthority: ( BIND | COPY | EXECUTE | RUN);

/*
 It turns out that if the collectionID is in quotes and the
 packageName is in quotes that they and
 the DOT don't match
...
grantee: (authorizationName | (ROLE roleName) | PUBLIC);
...
statementInformation: (
		(
			statementInformationVariableEquate (
				COMMA statementInformationVariableEquate
			)*
		)
...
/*
 ...
 are here because even though they
 aren't included in what the
 documentation lists as statement-information-item-name, they
 are
 included individually where those items are listed.
 
 I suspect, from the syntax diagram, one can
 only list them 
 alone, without other items. Regardless, I'm including them
 here for simplicity's
 sake.
 */
...
conditionInformation: (
		CONDITION (variable | INTEGERLITERAL) conditionInformationVariableEquate (
			COMMA conditionInformationVariableEquate
		)*
	);
...
combinedInformation: (
		variable EQ ALL combinedInformationOption (
			COMMA combinedInformationOption
		)*
	);
...
rowsetPositioned: (
		...
		| (CURRENT ROWSET)
		| (
			ROWSET STARTING AT (ABSOLUTE | RELATIVE) (
				hostVariable
				| INTEGERLITERAL
			)
		)
...
/*
 ...
 host-variable-name,
 SQL-parameter-name, SQL-variable-name, 
 transition-variable-name} all of which conform to the
 variableName rule
 save for host-variable-name; thus we confine the rule to just those two.
 */

fetchTargetVariable: (variable | hostVariable);

/*
 The syntax diagram in the documentation shows that both of these
 clauses are optional. In ANTLR
 terms, this means an error of the
 ...
 entire rule is optional in the
 fetchStatement rule. 
 */
multipleRowFetch: (
...
multipleRowFetchIntoClause: (
		(INTO hostVariable (COMMA hostVariable)*)
		| ((INTO | USING) DESCRIPTOR descriptorName)
	);
...
explainPackageClause: ( PACKAGE packageScopeSpecification);
...
packageScopeSpecification: (
		COLLECTION collectionName PACKAGE packageScopePackageName (
			VERSION versionName
		)? (COPY NONNUMERICLITERAL)?
	);

collectionName: (: hostVariable | NONNUMERICLITERAL);
...
/*
 ...
 host-variable and a
 host-variable-array. The first option in
 the sourceRowData rule allows either, but only for former
 is 
...
dropDatabaseClause: (DATABASE databaseName);
...
dropIndexClause: (INDEX indexName);

dropMaskClause: (MASK maskName);

dropPackageClause: (PACKAGE packageDesignator);

dropPermissionClause: (PERMISSION permissionName);

dropProcedureClause: (PROCEDURE procedureName RESTRICT?);

dropRoleClause: (ROLE roleName RESTRICT?);

dropSequenceClause: (SEQUENCE sequenceName RESTRICT?);

dropStogroupClause: (STOGROUP stogroupName);

dropSynonymClause: (SYNONYM synonym);

dropTableClause: (TABLE tableName);
 ...
 */
dropTriggerClause: (TRIGGER triggerName RESTRICT?);

dropTrustedContextClause: (TRUSTED CONTEXT contextName);

dropTypeClause: (TYPE typeName RESTRICT?);

dropVariableClause: (VARIABLE variableName RESTRICT?);

dropViewClause: (VIEW viewName);

packageDesignator: (
		collectionID DOT packageName (VERSION? versionID)?
	);

describeUsingOption: (USING (NAMES | LABELS | ANY | BOTH));
...
trustedContextEnableDisableClause: (DISABLE | ENABLE);

trustedContextDefaultSecurityLabelClause: (
		(NO DEFAULT SECURITY LABEL)
		| (DEFAULT SECURITY LABEL seclabelName)
	);
		...
		ATTRIBUTES LPAREN (
			(
				trustedContextAttribute1 (
					COMMA trustedContextAttribute1
				)*
			)
			| (
				trustedContextAttribute2 (
					COMMA trustedContextAttribute2
				)*
			)
		) RPAREN
	);
...
trustedContextWithUseForClause: (
		WITH USE FOR trustedContextUseFor (
			COMMA trustedContextUseFor
		)*
	);
...
trustedContextAttribute2: ( (JOBNAME jobnameValue));
...
triggerDefinition: (
		triggerActivationTime triggerEvent ON tableName triggerReferencingPhrase? triggerGranularity
			MODE_ DB2SQL triggerDefinitionOption? triggeredAction
	);

triggerAdvancedDefinition: (
		versionOption? triggerAdvancedActivationTime triggerEvent ON tableName
			triggerAdvancedReferencingPhrase? triggerGranularity triggerAdvancedDefinitionOptionList
...
triggerGranularity: ( (FOR EACH STATEMENT) | (FOR EACH ROW));
...
triggerDefinitionOption: ( (NOT SECURED) | SECURED);
 ...
 */
customVolatileClause: (NOT? VOLATILE CARDINALITY?);

/*
 Restructured per observation by Michel A. G. Poppema 2023-05-26.
 Added columnDefinitionOptions as
 they can be in any order, previous
 implementation enforced an order incorrectly.
 */
createTableColumnDefinition: (
		columnName dataType? columnDefinitionOptions*
	);

/*
 Added forDataQualifier as it can appear after NOT NULL and
 still be syntactically valid, contrary
 to the syntax diagram.
 ...
 */
validprocClause: (VALIDPROC (programName | NULL));

auditClause: (AUDIT (NONE | CHANGES | ALL));

obidClause: (OBID INTEGERLITERAL);

dataCaptureClause: (DATA CAPTURE (NONE | CHANGES));

restrictOnDropClause: (WITH RESTRICT ON DROP);

ccsidClause1: (CCSID (ASCII | EBCDIC | UNICODE)?);

ccsidClause2: (CCSID INTEGERLITERAL);

cardinalityClause: (NOT? VOLATILE CARDINALITY?);

appendClause: (APPEND (YES | NO));

memberClause: (MEMBER CLUSTER);

trackmodClause: (TRACKMOD (YES | NO));

pagenumClause: (PAGENUM (RELATIVE | ABSOLUTE));

/*
 Arguments to programName are options, noted when testing new
 implementation of
 createTableColumnDefinition 2023-05-26.
 */
fieldprocClause: (
		FIELDPROC programName (
			LPAREN literal (COMMA literal)* RPAREN
		)?
	);

asSecurityLabelClause: (AS SECURITY LABEL);

implicitlyHiddenClause: (IMPLICITLY HIDDEN_);

inlineLengthClause: (INLINE LENGTH INTEGERLITERAL);
...
copyOptionRowChangeTimestamp: (
		(EXCLUDING | INCLUDING) ROW CHANGE TIMESTAMP (
			COLUMN ATTRIBUTES
		)?
	);
...
copyOptionXmlTypeModifiers: (EXCLUDING XML TYPE MODIFIERS);
 ...
 */
asResultTable: (
		(LPAREN (columnName (COMMA columnName)*) RPAREN)? AS LPAREN fullSelect RPAREN WITH NO DATA
	);

declareGlobalTemporaryTableAsResultTable: (
		AS LPAREN fullSelect RPAREN WITH NO DATA
	);

createTableMaterializedQueryDefinition: (
		(LPAREN columnName (COMMA columnName)* RPAREN)? AS materializedQueryDefinition
	);
...
//#KMG
organizationClause: (
		ORGANIZE BY HASH UNIQUE LPAREN columnName (
			COMMA columnName
		)* RPAREN (HASH SPACE INTEGERLITERAL? sqlidentifier)?
	)
	/*{
...
parameterDeclaration1: (
		parameterName? (
			(functionDataType (AS LOCATOR)?)
			| (TABLE LIKE tableName (AS LOCATOR)?)
		)
	);

parameterDeclaration2: ( parameterName functionDataType);

/*
 I cannot find where it is documented that (IN | OUT | INOUT)? is allowed
 following the
 procedureDataType but apparently it is.
 */
parameterDeclaration3: (
		(IN | OUT | INOUT)? parameterName? procedureDataType (
			AS LOCATOR
		)? (IN | OUT | INOUT)?
	);
...
/*
 ...
 Erwin Veenstra 2024-10-29. It appears
...
languageOption1: (LANGUAGE SQL);

languageOption2: (LANGUAGE (ASSEMBLE | C_ | COBOL | PLI));

languageOption3: (LANGUAGE (ASSEMBLE | C_ | COBOL | JAVA | PLI));
...
parameterStyleOption1: (PARAMETER STYLE SQL);

parameterStyleOption2: (PARAMETER STYLE (SQL | JAVA));
...
parameterStyleOption3: (
		PARAMETER STYLE (
			...
			| GENERAL
			| (SIMPLE CALL)
			| ((GENERAL | (SIMPLE CALL)) WITH NULLS)
			| JAVA
		)
	);

deterministicOption: ((NOT? DETERMINISTIC) | (NOT? VARIANT));
...
nullInputOption2: ((CALLED ON NULL INPUT) | (NULL CALL));
...
nullInputOption4: (CALLED ON NULL INPUT);

debugOption: ((DISALLOW | ALLOW | DISABLE) DEBUG MODE_);

sqlDataOption1: ((READS SQL DATA) | (CONTAINS SQL));

sqlDataOption2: ((READS SQL DATA) | (CONTAINS SQL) | (NO SQL));
...
externalActionOption: (NO? EXTERNAL ACTION);
...
scratchpadOption: ((NO SCRATCHPAD) | (SCRATCHPAD INTEGERLITERAL));

finalCallOption: (NO? FINAL CALL);

parallelOption1: (DISALLOW PARALLEL);

parallelOption2: ((ALLOW | DISALLOW) PARALLEL);

dbinfoOption: (NO? DBINFO);

cardinalityOption: (CARDINALITY INTEGERLITERAL);

collectionIdOption: ((NO COLLID) | (COLLID collectionID));

/*
 This was formerly (and incorrectly) used in createFunctionStatementExternalScalarOptions
 and
 createFunctionStatementExternalTableOptions. It doesn't match the ALTER statement
 for either
 function and doesn't match the explanatory text in the documentation. My
 mistake was I only looked
 at the syntax diagram when I was creating the grammar.
 
 Error found by Erwin Veenstra
 2024-10-24.
 
 wlmEnvironmentOption1
 : (WLM ENVIRONMENT (identifier | (LPAREN identifier
 RPAREN)))
 ;
 */
...
wlmEnvironmentOption2: (
		WLM ENVIRONMENT (
			identifier
			| (LPAREN identifier COMMA SPLAT RPAREN)
		)
	);
...
asuTimeOption: (ASUTIME ((NO LIMIT) | (LIMIT INTEGERLITERAL)));

stayResidentOption: (STAY RESIDENT (NO | YES));

programTypeOption: (PROGRAM TYPE (SUB | MAIN));

securityOption: (SECURITY (DB2 | USER | DEFINER));

stopAfterFailureOption: (
		(STOP AFTER SYSTEM DEFAULT FAILURES)
		| (STOP AFTER INTEGERLITERAL FAILURES)
		| (CONTINUE AFTER FAILURE)
	);

runOptionsOption: (RUN OPTIONS runTimeOptions);

commitOnReturnOption: (COMMIT ON RETURN (YES | NO));

specialRegistersOption: ((INHERIT | DEFAULT) SPECIAL REGISTERS);

specialRegistersOption2: (INHERIT SPECIAL REGISTERS);

dispatchOption: (STATIC DISPATCH);

securedOption: (NOT? SECURED);

specificNameOption1: (SPECIFIC specificName?);

specificNameOption2: (SPECIFIC specificName);
...
parameterOption2: (PARAMETER ccsidClause1);
...
/*
 ...
 This allows the
 forDataQualifier and ccsidClause1 phrases to be
 reversed from how they are defined in
 functionBuiltInType.
 */
inlineSqlScalarFunctionDefinition: (
		RETURNS functionDataType (forDataQualifier ccsidClause1)? languageOption1?
			createFunctionStatementInlineSqlScalarOptions+ sqlRoutineBody
	);
 ...
 */
compiledSqlScalarFunctionDefinition: (
		RETURNS functionDataType (forDataQualifier ccsidClause1)? versionOption?
			createFunctionStatementCompiledSqlScalarOptions+ sqlRoutineBody
	);
...
/*
 ...
 simply incorrect according to the
 documentation. Found by Martijn
 Rutte 2023-07-06.
 */
sqlTableFunctionDefinition: (
		RETURNS TABLE LPAREN columnName functionDataType (
			COMMA columnName functionDataType
		)* RPAREN createFunctionStatementSqlTableOptions+ sqlTableReturnStatement
	);
...
sqlTableReturnStatement: (
		(
			BEGIN sqlplCompoundAtomicClause? RETURN (
				WITH commonTableExpression (
					COMMA commonTableExpression
				)*
			)? fullSelect SEMICOLON? END SEMICOLON?
		)
		| (
			RETURN (
				WITH commonTableExpression (
			...
			)? fullSelect SEMICOLON?
		)
	);

sequenceAlias: ( aliasName FOR SEQUENCE sequenceName);

tableAlias: ( aliasName FOR TABLE? tableName);

authorization: (USER hostVariable USING hostVariable);

/*
 Alternate syntax allows omission of FROM. (WHERE searchCondition) should
 be optional, i.e. DELETE
 T1; is a valid statement.
...
searchedDelete: (
		...
		)? (WHERE searchCondition)? fetchClause? (
			isolationClause
...
searchedUpdate: (
		UPDATE tableName periodClause? AS? correlationName? includeColumns? SET assignmentClause (
			WHERE searchCondition
		)? (isolationClause | skipLockedDataClause)* querynoClause?
	);

positionedUpdate: (
		UPDATE tableName AS? correlationName? SET assignmentClause WHERE CURRENT OF cursorName (
			FOR ROW (hostVariable | INTEGERLITERAL) OF ROWSET
		)?
	);

sourceValues: (
		LPAREN VALUES (valuesSingleRow | valuesMultipleRow) RPAREN AS? correlationName LPAREN
			columnName (COMMA columnName)* RPAREN
	);

valuesSingleRow: (
		valuesList3
		| (LPAREN valuesList3 (COMMA valuesList3)* RPAREN)
	);

valuesMultipleRow: (
		valuesList4
		| (LPAREN valuesList4 (COMMA valuesList4)* RPAREN) FOR (
			hostVariable
			| INTEGERLITERAL
		) ROWS
	);

matchingCondition: ( NOT? MATCHED (AND searchCondition)?);
...
assignmentClause: (
		(
			columnName EQ valuesList1 (
				COMMA columnName EQ valuesList1
			)*
		)
		| (
			LPAREN columnName (COMMA columnName)* RPAREN EQ LPAREN (
				(valuesList1 (COMMA valuesList1)*)
				| fullSelect
			) RPAREN
		)
...
setAssignmentClause: (
		...
		| (
			setAssignmentTargetVariable EQ valuesList1 (
				COMMA setAssignmentTargetVariable EQ valuesList1
			)*
		)
		| (
			LPAREN setAssignmentTargetVariable (
				COMMA setAssignmentTargetVariable
			)* RPAREN EQ LPAREN (
...
insertOperation: (
		INSERT LPAREN columnName (COMMA columnName)* RPAREN VALUES (
			valuesList1
			| (LPAREN valuesList1 (COMMA valuesList1)* RPAREN)
		)
	);
...
valuesList2: (expression | hostVariable | DEFAULT | NULL);

valuesList3: (expression | NULL);

valuesList4: (expression | hostVariable | NULL);

includeColumns: (
		INCLUDE LPAREN columnName dataType (
			COMMA columnName dataType
		)* RPAREN
	);
...
multipleRowInsert: (
		VALUES (
			valuesList2
			| (LPAREN valuesList2 (COMMA valuesList2)* RPAREN)
		) (FOR (hostVariable | INTEGERLITERAL) ROWS)? (
			ATOMIC
			| notAtomicPhrase
		)
...
//
bufferpoolOption: (BUFFERPOOL bpName);

closeOption: (CLOSE (YES | NO));

copyOption: (COPY (YES | NO));
...
dssizeOption: (
		...
		string sqlident = $SQLIDENTIFIER.text;
			if (!sqlident.ToUpper().EndsWith("G")) {
		...
		try {
			dssizeVal = Int32.Parse(sqlidentVal.Trim());
			if ((dssizeVal == 0) || ((dssizeVal & -dssizeVal) != dssizeVal)) {
...
/*
 ...
 request
 submitted to IBM on 05-Aug-2022.
 
 IBM responded to the documentation correction request on
 17-Aug-2022, noting that in _any_ place the syntax
 <integer>[K|M|G] was allowed, whitespace was
 allowed
 between the <integer> and the [K|M|G]. IBM indicated they
 would be updating their
 documentation to reflect this.
 
 All such changes tagged with #KMG
 */
piecesizeOption: (PIECESIZE INTEGERLITERAL? sqlidentifier);

clusterOption: (NOT? CLUSTER);

paddedOption: (NOT? PADDED);

compressOption: (COMPRESS ((YES (FIXEDLENGTH | HUFFMAN)?) | NO));

defineOption: (DEFINE (YES | NO));

locksizeOption: (
		LOCKSIZE (ANY | TABLESPACE | TABLE | PAGE | ROW | LOB)
	);

lockmaxOption: (LOCKMAX (SYSTEM | INTEGERLITERAL));

enableDisableOption: (ENABLE | DISABLE);
...
/*
 ...
 syntax."
 */
loggedOption: ((NOT? LOGGED) | (LOG NO) | (LOG YES));

notAtomicPhrase: (NOT ATOMIC CONTINUE ON SQLEXCEPTION);

/*
 2022-11-04 Issue 125 Changed usingSpecification1 to usingBlock for consistency.
 2023-05-24
 Alternate syntax says ALTER is optional.
 */
alterIndexPartitionOptions: (
...
gbpcacheSpecification: (GBPCACHE (CHANGED | ALL | SYSTEM | NONE));
...
partitionElement: (
		(PARTITION | PART) INTEGERLITERAL (
			((ENDING AT?) | VALUES) LPAREN (
				...
				| MAXVALUE
				| MINVALUE
			) (COMMA (literal | MAXVALUE | MINVALUE))* RPAREN INCLUSIVE?
		)?
	);
applCompatValue: APPLCOMPAT_LEVEL;

functionLevel: sqlidentifier;

functionParameterType: (functionDataType (AS LOCATOR)?);

functionDataType: (functionBuiltInType | distinctTypeName);
...
functionBuiltInType: (
		...
		| TIME
		| (
			TIMESTAMP integerInParens? (
				(WITH | WITHOUT) ((TIME ZONE) | TIMEZONE)
			)?
		)
		| ROWID
		| XML
...
/*
 ...
 (ccsidClause1 | forDataQualifier)*
 to allow the clauses to be
 in any order. 2023-05-24
 
 Changed TIME ZONE to ((TIME ZONE) |
 TIMEZONE) per 
 Maarten van Haasteren 2024-11-18.
 */
...
sourceDataType: procedureBuiltinType;
...
/*
 ...
 to use their parser rule equivalents.
 Hopefully this
...
/*
 ...
 external (mandatory languageOption5) 
 and native SQL (optional languageOption1) stored
 procedures.
 */
...
versionOption: (VERSION (identifier | literal));

commitOnReturnOptionSQLPL: (AUTONOMOUS | commitOnReturnOption);

schemaQualifierOption: (QUALIFIER schemaName);

currentDataOption: (CURRENT DATA (YES | NO));

immediateWriteOption: ((WITH | WITHOUT) IMMEDIATE WRITE);

explainOption: ((WITH | WITHOUT) EXPLAIN);

reoptOption: (REOPT (NONE | ALWAYS | ONCE));

packageOwnerOption: (
		PACKAGE OWNER authorizationName (AS (ROLE | USER))?
	);

deferPrepareOption: ((DEFER PREPARE) | (NODEFER PREPARE));

degreeOption: (DEGREE (ANY | INTEGERLITERAL));
...
isolationLevelOption: (ISOLATION LEVEL (CS | RS | RR | UR));

keepDynamicOption: ((WITH | WITHOUT) KEEP DYNAMIC);

opthintOption: (OPTHINT NONNUMERICLITERAL);
...
getAccelArchiveOption: (GET_ACCEL_ARCHIVE (NO | YES));

accelerationOption: (ACCELERATION WAITFORDATA NUMERICLITERAL);

releaseAtOption: (RELEASE AT (COMMIT | DEALLOCATE));

businessTimeSensitiveOption: (BUSINESS_TIME SENSITIVE (YES | NO));

systemTimeSensitiveOption: (SYSTEM_TIME SENSITIVE (YES | NO));

archiveSensitiveOption: (ARCHIVE SENSITIVE (YES | NO));

applcompatOption: (APPLCOMPAT applCompatValue);

validateOption: (VALIDATE (RUN | BIND));

roundingOption: (ROUNDING roundingOptionItem);
...
dateFormatOption: (DATE FORMAT dateTimeFormatOptionItem);

dateTimeFormatOptionItem: (ISO | EUR | USA | JIS | LOCAL);

timeFormatOption: (TIME FORMAT dateTimeFormatOptionItem);

decimalOption: (
		DECIMAL LPAREN INTEGERLITERAL (COMMA INTEGERLITERAL)? RPAREN
	);

forUpdateOption: (FOR UPDATE CLAUSE (REQUIRED | OPTIONAL));

concentrateStatementsOption: (
		CONCENTRATE STATEMENTS (OFF | (WITH LITERALS))
	);

acceleratorOption: (ACCELERATOR acceleratorName);

procedureDataType: (procedureBuiltinType | distinctTypeName);
...
asTypeOption: (AS sequenceDataType);

/*
 Non-integer literals allowed, noted by Martijn Rutte 2023-05-23.
 */
startOption: (START WITH (NUMERICLITERAL | INTEGERLITERAL));

restartOption: (RESTART (WITH INTEGERLITERAL)?);

incrementOption: (INCREMENT BY (NUMERICLITERAL | INTEGERLITERAL));

minvalueOption: (
		NOMINVALUE
		| (NO MINVALUE)
		| (MINVALUE (NUMERICLITERAL | INTEGERLITERAL))
	);

maxvalueOption: (
		NOMAXVALUE
		| (NO MAXVALUE)
		| (MAXVALUE (NUMERICLITERAL | INTEGERLITERAL))
	);

cycleOption: (NOCYCLE | (NO? CYCLE));

cacheOption: (NOCACHE | (NO CACHE) | (CACHE INTEGERLITERAL));

orderOption: (NOORDER | (NO? ORDER));

keyLabelOption: ((NO KEY LABEL) | (KEY LABEL keyLabelName));

dataclasOption: (DATACLAS dcName);

mgmtclasOption: (MGMTCLAS mcName);

storclasOption: (STORCLAS scName);
...
alterStogroupOptionList: (
		...
		| (
			ADD VOLUMES LPAREN NONNUMERICLITERAL (
				COMMA NONNUMERICLITERAL
			)* RPAREN
		)
		| (
		...
		| (
			REMOVE VOLUMES LPAREN NONNUMERICLITERAL (
				COMMA NONNUMERICLITERAL
			)* RPAREN
		)
		| keyLabelOption
...
/*
 ...
 ...as it seems to be allowed on its own. Noted by Martijn
 Rutte 2023-01-10.
 ...
 ...as it seems to be allowed on its own.
 Noted by Martijn Rutte 2023-05-23.
 */
alterTableOptionList: (
		...
		| (
			DROP (
				(PRIMARY KEY)
				| (
					(UNIQUE | (FOREIGN KEY) | CHECK | CONSTRAINT) constraintName
				)
			)
		)
...
/*
 ...
 reference usingSpecification2 instead of
 usingBlock.
 2023-01-10 Martijn Rutte noticed this rule should allow
 pagenumClause on its own and
 not just as part of
...
/*
 ...
 is at odds with at least one
 example and arguably with the narrative.
 
 More specifically, it is unclear where the ALTER
 keyword is required,
...
trustedContextOptionList: (
		...
		| (
			ALTER ATTRIBUTES LPAREN alterAttributesOptions (
				COMMA alterAttributesOptions
			)* RPAREN
		)
		| (
			ADD ATTRIBUTES LPAREN addAttributesOptions (
				COMMA addAttributesOptions
			)* RPAREN
		)
		| (
			DROP ATTRIBUTES LPAREN dropAttributesOptions (
				COMMA dropAttributesOptions
			)* RPAREN
		)
		| userClause
	);
...
createIndexOptionList: (
		...
		| (
			(PARTITION BY RANGE?)? LPAREN partitionElement (
				usingSpecification2
				| freeSpecification
				| gbpcacheSpecification
				| dssizeOption
			)* (
				COMMA partitionElement (
					usingSpecification2
					| freeSpecification
					| gbpcacheSpecification
					| dssizeOption
...
inDatabaseOption: (IN databaseName);

segsizeOption: (SEGSIZE INTEGERLITERAL);

numpartsOption: (NUMPARTS INTEGERLITERAL);
			...
			(
				LPAREN (
					partitionByRangePartitionPhrase (
						COMMA partitionByRangePartitionPhrase
					)*
				)* RPAREN
			)
			| pagenumClause
...
insertAlgorithmOption: (INSERT ALGORITHM INTEGERLITERAL);

maxrowsOption: (MAXROWS INTEGERLITERAL);

maxpartitionsOption: (MAXPARTITIONS INTEGERLITERAL);
...
/*
 ...
 suggestion by Martijn Rutte 2022-12-06.
 More concise
...
/*
 ...
 grammar it is simply a quoted
 string. It probably warrants a grammar
 of its own.
 */
xmlPatternClause: NONNUMERICLITERAL;
...
userClause: (
		(
			ADD USE FOR userClauseAddOptions (
				COMMA userClauseAddOptions
			)*
		)
		| (
			REPLACE USE FOR userClauseReplaceOptions (
				COMMA userClauseReplaceOptions
			)*
		)
		| (
			DROP USE FOR userClauseDropOptions (
				COMMA userClauseDropOptions
			)*
		)
	);
...
userClauseReplaceOptions: (userClauseAddOptions);
...
useOptions: (
		(ROLE roleName)? (SECURITY LABEL seclabelName)? (
			WITH
			| WITHOUT
		) AUTHENTICATION
	);
...
freeBlock: (
		(
			...
			| (
				PCTFREE (
					INTEGERLITERAL? FOR UPDATE INTEGERLITERAL
				)?
			)
		)
	);
...
gbpcacheBlock: ( GBPCACHE (CHANGED | ALL | SYSTEM | NONE));

aliasDesignator: (PUBLIC? ALIAS aliasName FOR (TABLE | SEQUENCE));
...
functionDesignator: (
		(
			FUNCTION functionName (
				LPAREN (parameterType (COMMA parameterType)*)? RPAREN
			)?
		)
		| (SPECIFIC FUNCTION specificName)
	);
...
columnConstraint: (referencesClause | checkConstraint);
...
/*
 ...
 ...optional as it was not by by
 mistake. Noted by Martijn Rutte 2023-01-10.
 */
generatedClause: (
		(
...
asIdentityClause: (
		AS IDENTITY (
			LPAREN asIdentityClauseOptionList (
				COMMA? asIdentityClauseOptionList
			)* RPAREN
		)?
	);
...
asRowTransactionStartIDClause: (AS TRANSACTION START ID);

asRowTransactionTimestampClause: (AS ROW (BEGIN | START | END));
...
columnAlteration: (columnName columnAlterationOptionList+);

columnAlterationOptionList: (
		(
			SET DATA TYPE alteredDataType (
				INLINE LENGTH INTEGERLITERAL
			)?
		)
		| (SET defaultClause)
...
/*
 ...
 thereof. The dataType rule includes a provision 
 for CCSID on CHAR, VARCHAR, CLOB, GRAPHIC,
 VARGRAPHIC, and DBCLOB types
 ...
 */
alteredDataType: dataType;

/*
 The difference between dataType and castDataType is in the coding
 of the CCSID and FOR ... DATA
 qualifiers. Sneaky.
 */
dataType: (builtInType | distinctTypeName);
...
sequenceDataType: (sequenceBuiltInType | distinctTypeName);
...
xmlTypeModifier: (
		XMLSCHEMA xmlSchemaSpecification (ELEMENT xmlElementName)? (
...
xmlSchemaSpecification: (
		...
		| (
			((URL targetNamespace) | (NO NAMESPACE)) (
 ...
 */
xmlElementName: (identifier | literal);

piName: (NONNUMERICLITERAL);

registeredXmlSchemaName: ( SYSXSR DOT sqlidentifier);

targetNamespace: (NONNUMERICLITERAL);

schemaLocation: (NONNUMERICLITERAL);
...
uniqueConstraint: (
		(CONSTRAINT constraintName)? ((PRIMARY KEY) | UNIQUE) LPAREN columnName (
			COMMA columnName
		)* (COMMA BUSINESS_TIME WITHOUT OVERLAPS)? RPAREN
	);
...
referentialConstraint: (
		...
		) LPAREN columnName (PERIOD BUSINESS_TIME)? (
			COMMA columnName (PERIOD BUSINESS_TIME)?
		)* RPAREN referencesClause
	);
...
partitioningClause: (
		PARTITION BY (
			(
				RANGE? LPAREN partitionExpression (
					COMMA partitionExpression
				)* RPAREN LPAREN (
					(
						partitioningClauseElement (
							COMMA partitioningClauseElement
						)*
					)
					| (
						partitioningClauseElementAlternateSyntax (
							COMMA partitioningClauseElementAlternateSyntax
						)*
					)
				) RPAREN
			)
...
/*
 ...
 ...optional because it wasn't and it should be. Found by Martijn Rutte
 2022-01-09.
 */
partitionExpression: ( columnName (NULLS LAST)? (ASC | DESC)?);
...
/*
 ...
 it references. Found by
 Martijn Rutte 2022-01-09.
 */
partitionLimitKey: (literal | MAXVALUE | MINVALUE);

/*
 The partitionHashSpace rule can be before the INCLUSIVE token in
 the create table statement, or
 after the INCLUSIVE token in the
 alter table statement. Also, it's deprecated as of Db2 12.
 */
partitioningPhrase: (
		ENDING AT? LPAREN partitionLimitKey (
			COMMA partitionLimitKey
		)* RPAREN partitionHashSpace? INCLUSIVE? partitionHashSpace?
	);

//deprecated as of Db2 12 #KMG
partitionHashSpace: (HASH SPACE INTEGERLITERAL? sqlidentifier)
	; //{notifyErrorListeners("HASH SPACEs are deprecated as of DB2 12");}

//deprecated as of Db2 12 #KMG
alterHashOrganization: (
		(
			ADD ORGANIZE BY HASH UNIQUE LPAREN columnName (
				COMMA columnName
			)* RPAREN HASH SPACE INTEGERLITERAL? sqlidentifier
		)
		| (
...
partitioningClauseElementAlternateSyntax: (
		(PARTITION | PART) INTEGERLITERAL (
			VALUES LPAREN partitionLimitKey (
				COMMA partitionLimitKey
			)* RPAREN
		) INCLUSIVE?
	);

partitionClause: ( partitioningPhrase | partitionHashSpace);

rotatePartitionClause: (partitioningPhrase RESET);

extraRowOption: (ON DELETE ADD EXTRA ROW);

materializedQueryDefinition: (
		LPAREN fullSelect RPAREN refreshableTableOptions
	);

materializedQueryAlteration: (SET refreshableTableOptionsList+);

refreshableTableOptions: (
		dataInitiallyDeferredPhrase refreshDeferredPhrase refreshableTableOptionsList*
	);

dataInitiallyDeferredPhrase: (DATA INITIALLY DEFERRED);

refreshDeferredPhrase: (REFRESH DEFERRED);
...
externalProgramName: (identifier | NONNUMERICLITERAL);
...
runTimeOptions: NONNUMERICLITERAL;

comparisonOperator: (EQ | GT | LT | GE | LE | NE);

operator: (SPLAT | PLUS | MINUS | SLASH | CONCAT | CONCATOP);

/*
 Note that arrayConstructor must precede arrayElementSpecification
 in the expression rule.
 Otherwise the former is mistaken for
 the latter.
 
 Note that sequenceReference must precede
 columnName in the expression
 rule. Otherwise the former is mistaken for the latter.
 
 Made
 expression optional in ((operator | INTEGERLITERAL) expression?)
 per Martijn Rutte 2023-05-23.
 This allows for...
 
 SET C1 = SUBSTR(C2,LENGTH(C2)-1);
 
 ...due to the -1 being output from the
 lexer as one token (INTEGERLITERAL)
...
scalarFunctionInvocation: (
		...
		| interpretFunction
		| (
			(schemaName DOT)? scalarFunction LPAREN (
				expression (COMMA expression)*
			)? RPAREN (AS NONNUMERICLITERAL)?
		)
	);
 ...
 */
trimFunction: (
		TRIM LPAREN (trimFunctionIndicator? literal? FROM)? expression RPAREN
	);
...
stripFunction: (
		STRIP LPAREN (trimFunctionIndicator? literal? FROM)? expression RPAREN
	);
 ...
 */
extractFunction: (
		EXTRACT LPAREN (datePrefix | timePrefix | timezonePrefix) FROM expression RPAREN
	);
...
interpretFunctionDataType: ( integerDataType | charDataType);

integerDataType: ( SMALLINT | INTEGER | INT | BIGINT);
...
aggregateFunctionInvocation: (
		...
		| xmlaggFunction
		| (
			(schemaName DOT)? aggregateFunction LPAREN (
				ALL
				| DISTINCT
			)? (expression | SPLAT) RPAREN
		)
	);

regressionFunctionInvocation: (
		(schemaName DOT)? regressionFunction LPAREN expression COMMA expression RPAREN
	);
 ...
 */
externalFunctionInvocation: (
		(schemaName DOT)? sqlidentifier LPAREN (
			expression (COMMA expression)*
		)? RPAREN
	);
...
/*
 ...
 failed on a scalarFullSelect due to its
 requirement
 that the fullSelect be in parens. The expression
 had to be in parens here, then the
 fullSelect also
 had to be in parens, so two sets of parens were
 required by this rule when only
 one was required
 by DB2z. 2024-12-12
 */
labeledDuration: (
...
/*
 ...
 that XMLCAST invocations did not
 parse correctly. 2023-01-24
 */
xmlCastSpecification:
	XMLCAST LPAREN (expression | NULL | parameterMarker) AS dataType RPAREN;

xmlParseSpecification:
	XMLPARSE LPAREN DOCUMENT expression (
...
arrayIndex: expression (operator? expression)*;
...
orderedOlapSpecification:
	olapSpecificationFunction OVER LPAREN windowPartitionClause? windowOrderClause RPAREN;
...
lagFunction:
	LAG LPAREN expression (
		COMMA INTEGERLITERAL (
			COMMA expression (
				COMMA ((RESPECT NULLS) | (IGNORE NULLS))
			)?
		)? RPAREN
	);
...
leadFunction:
	LEAD LPAREN expression (
		COMMA INTEGERLITERAL (
			COMMA expression (COMMA respectNullsClause)?
		)? RPAREN
	);

respectNullsClause: ((RESPECT NULLS) | (IGNORE NULLS));
...
windowOrderClause:
	ORDER BY expression windowOrderClauseQualifier? (
		COMMA expression windowOrderClauseQualifier?
	)*;

windowOrderClauseQualifier: (ASC | DESC) (NULLS (FIRST | LAST))?;
...
aggregationSpecification: (
		aggregateFunctionInvocation
		| olapColumnFunction
	) OVER LPAREN windowPartitionClause? (
		(
...
ratioToReportFunction: (RATIO_TO_REPORT LPAREN expression RPAREN);

listaggFunction: (
		LISTAGG LPAREN (ALL | DISTINCT)? expression (
			COMMA NONNUMERICLITERAL
		)? RPAREN (
			WITHIN GROUP LPAREN ORDER BY sortKey (ASC | DESC)? (
...
cumeDistFunction: (
		CUME_DIST LPAREN expression (COMMA expression)* RPAREN WITHIN GROUP LPAREN
			aggregateOrderByClause RPAREN
	);

percentileContFunction: (
		PERCENTILE_CONT LPAREN expression RPAREN WITHIN GROUP LPAREN ORDER BY expression (
			ASC
			| DESC
		)? RPAREN
	);
...
percentileDiscFunction: (
		PERCENTILE_DISC LPAREN expression RPAREN WITHIN GROUP LPAREN ORDER BY expression (
			ASC
			| DESC
		)? RPAREN
	);
...
percentRankFunction: (
		PERCENT_RANK LPAREN expression (COMMA expression)* RPAREN WITHIN GROUP LPAREN
			aggregateOrderByClause RPAREN
	);
...
xmlaggOrderByOption: (expression (ASC | DESC)?);

aggregateOrderByClause: (
		ORDER BY aggregateOrderByOption (
			COMMA aggregateOrderByOption
		)*
	);
...
windowAggregationGroupClause: (ROWS | RANGE) (
		groupStart
		| groupBetween
		| groupEnd
	);

groupStart: (unboundedPreceding | boundedPreceding | currentRow);

groupBetween: BETWEEN groupBound1 AND groupBound2;

groupEnd: (unboundedFollowing | boundedFollowing);
...
boundedPreceding: INTEGERLITERAL PRECEDING;

boundedFollowing: INTEGERLITERAL FOLLOWING;
...
aiAnalogyFunction: (
		AI_ANALOGY LPAREN aiAnalogyFunctionSource1 COMMA aiAnalogyFunctionTarget1 COMMA
			aiAnalogyFunctionSource2 COMMA aiAnalogyFunctionTarget2 RPAREN
	);
...
aiAnalogyFunctionSource: aiFunctionExpression;

aiAnalogyFunctionTarget: aiFunctionExpression;

aiAnalogyFunctionSource1: aiAnalogyFunctionSource;

aiAnalogyFunctionSource2: aiAnalogyFunctionSource;

aiAnalogyFunctionTarget1: aiAnalogyFunctionTarget;

aiAnalogyFunctionTarget2: aiAnalogyFunctionTarget;

aiCommonalityFunction: (
		AI_COMMONALITY LPAREN aiFunctionExpression RPAREN
	);

aiSemanticClusterFunction: (
		AI_SEMANTIC_CLUSTER LPAREN aiSemanticClusterMemberExpression COMMA
			aiSemanticClusterClusteringExpression (
			COMMA aiSemanticClusterClusteringExpression
		)* RPAREN
	);

aiSemanticClusterMemberExpression: aiFunctionExpression;

aiSemanticClusterClusteringExpression: expression;

aiSimilarityFunction: (
		AI_SIMILARITY LPAREN aiSimilarityExpression1 COMMA aiSimilarityExpression2 RPAREN
	);

aiSimilarityExpression: aiFunctionExpression;

aiSimilarityExpression1: aiSimilarityExpression;

aiSimilarityExpression2: aiSimilarityExpression;

xmlelementFunction: (
		XMLELEMENT LPAREN NAME xmlElementName (
			COMMA xmlnamespacesDeclaration
		)? (COMMA xmlattributesFunction)? (COMMA expression)* xmlFunctionOptionClause? RPAREN
	);

xmlforestFunction: (
		XMLFOREST LPAREN xmlnamespaceFunction? elementContentExpression (
			AS xmlElementName
		)? (COMMA elementContentExpression (AS xmlElementName)?)* xmlFunctionOptionClause? RPAREN
	);
...
xmlqueryFunction: (
		XMLQUERY LPAREN xqueryExpressionConstant (
			PASSING (BY REF)? xqueryArgument (
				COMMA xqueryArgument
			)*
		)? (RETURNING SEQUENCE (BY REF)?)? (EMPTY ON EMPTY)? RPAREN
	);

xmlattributesFunction: (
		XMLATTRIBUTES LPAREN expression AS NONNUMERICLITERAL (
			COMMA expression AS NONNUMERICLITERAL
		)* RPAREN
	);

xmlserializeFunction: (
		XMLSERIALIZE LPAREN CONTENT? expression AS dataType xmlserializeFunctionOptions* RPAREN
	);

xmlnamespaceFunction: (
		XMLNAMESPACES LPAREN xmlnamespaceOption (
			COMMA xmlnamespaceOption
		)* RPAREN
	);
...
xmlFunctionOptionClause: ( OPTION xmlFunctionOption+);
...
elementContentExpression: (expression);

xqueryExpressionConstant: (NONNUMERICLITERAL);

xqueryArgument: (
		xqueryContextItemExpression
		| (
			xqueryVariableExpression AS (
				identifier
				| NONNUMERICLITERAL
			)
		)
...
/*
 ...
 the IBM documentation for
 the XMLTABLE function, but if DB2z accepts
 it, we add it here.
 */
xmltableFunctionSpecification: (
		XMLTABLE LPAREN (xmlnamespacesDeclaration COMMA)? rowXqueryExpressionConstant (
			PASSING (BY REF)? rowXqueryArgument (
				COMMA rowXqueryArgument
			)*
		)? (
			SQL? COLUMNS (
...
rowXqueryExpressionConstant: (NONNUMERICLITERAL);
...
xqueryContextItemExpression: (expression);

xqueryVariableExpression: (expression);

/*
 Changed optionality of (defaultClause | (PATH columnXqueryExpressionConstant))
 from ? to * because
 you can have one, the other, both, or neither. IBM
 documentation says you can have one, the other,
 or neither. If DB2z accepts
 it we put it here.
 */
xmlTableRegularColumnDefinition: (
		columnName dataType (
			defaultClause
			| (PATH columnXqueryExpressionConstant)
		)*
	);
...
/*
 ...
 | (distinctTypeCastFunctionName LPAREN defaultClauseAllowables
 RPAREN))
 ...
 given the addition of scalarFunctionInvocation to
 defaultClauseAllowables.
 */
defaultClause: (
		WITH? DEFAULT (
...
defaultClause1: ( WITH? DEFAULT defaultClauseAllowables?);

/*
 Added scalarFunctionInvocation to allow what IBM documentation refers to
 as cast-function-name.
 Need noted by Martijn Rutte 2023-06-05.
 */
defaultClauseAllowables: (
		literal
		| SESSION_USER
...
distinctTypeCastFunctionName: (identifier DOT identifier);
...
/*
 ...
 expression 
 (COMMA
 INTEGERLITERAL (COMMA (CODEUNITS16 | CODEUNITS32 | OCTETS))?)?
 )
 ;
...
columnXqueryExpressionConstant: (NONNUMERICLITERAL);

xmlTableOrdinalityColumnDefinition: (columnName FOR ORDINALITY);
...
xmlnamespacesFunctionSpecification: (
		XMLNAMESPACES LPAREN xmlnamespacesFunctionArguments (
...
namespaceUri: NONNUMERICLITERAL;

namespacePrefix: NONNUMERICLITERAL;
...
/*
 ...
 Changed
 TIME ZONE to ((TIME ZONE) | TIMEZONE) per 
 Maarten van Haasteren 2024-11-18.
 
 */
timeZoneSpecificExpression:
	timeZoneExpressionSubset integerInParens? (
		(AT LOCAL)
		| (AT ((TIME ZONE) | TIMEZONE) timeZoneExpressionSubset)
		| ((WITH | WITHOUT) ((TIME ZONE) | TIMEZONE))
	);
 ...
 */
caseExpression:
	CASE (searchedWhenClause+ | simpleWhenClause) (
		(ELSE NULL)
...
searchCondition:
	NOT? (
		(predicate (SELECTIVITY NUMERICLITERAL)?)
...
arrayExistsPredicate:
	ARRAY_EXISTS LPAREN arrayExpression INTEGERLITERAL RPAREN;

betweenPredicate:
	expression NOT? BETWEEN expression AND expression;

distinctPredicate: expression IS NOT? DISTINCT FROM expression;

existsPredicate: EXISTS LPAREN fullSelect RPAREN;
...
nullPredicate: expression ((IS NOT? NULL) | ISNULL | NOTNULL);

xmlExistsPredicate:
	XMLEXISTS LPAREN NONNUMERICLITERAL (
		PASSING (BY REF)? expression (COMMA expression)*
	)? RPAREN;

arrayExpression: variable | castSpecification;

castSpecification:
	CAST LPAREN (expression | NULL | parameterMarker) AS castDataType RPAREN;

parameterMarker: QUESTIONMARK;

castDataType: ( castBuiltInType | distinctTypeName | arrayType);

/*
 Modified to allow forDataQualifier on its own per Martijn Rutte 2023-05-24.
 
 Changed TIME ZONE to
 ((TIME ZONE) | TIMEZONE) per 
 Maarten van Haasteren 2024-11-18.
...
castBuiltInType: (
		...
		| (
			(((CHARACTER | CHAR) VARYING?) | VARCHAR) (
				length
				| (LPAREN RPAREN)
			)? (ccsidQualifier | forDataQualifier)?
		)
		| (
			(((CHARACTER | CHAR) LARGE OBJECT) | CLOB) (
				length
...
/*
 ...
 as being part of the INTEGERLITERAL and
 becomes an SQLIDENTIFIER.
 ...
 correct. So we can see an INTEGERLITERAL,
 an INTEGERLITERAL followed by an sqlidentifier, or an
 sqlidentifier.
 */
//#KMG
length: (
		LPAREN (
			...
			| sqlidentifier
		) (CODEUNITS16 | CODEUNITS32 | OCTETS)? RPAREN
	);

ccsidQualifier: (
		CCSID (
			((ASCII | EBCDIC | UNICODE) forDataQualifier?)
			| INTEGERLITERAL
		)
	);

forDataQualifier: (FOR (SBCS | MIXED | BIT) DATA);

distinctTypeName: (schemaName DOT)? identifier;

arrayType: identifier;

literal: NUMERICLITERAL | NONNUMERICLITERAL | INTEGERLITERAL;

ccsidValue: INTEGERLITERAL;

columnName: (((correlationName | tableName) DOT)? identifier1);
...
beginColumnName: identifier;

endColumnName: identifier;

correlationName: identifier;

locationName
	: sqlidentifier; //	: (identifier | NUMERICLITERAL | INTEGERLITERAL) (DOT? (identifier | NUMERICLITERAL | INTEGERLITERAL))*

schemaName: identifier1;
...
cloneTableName: tableName;

archiveTableName: tableName;

viewName: ((locationName DOT schemaName DOT) | (schemaName DOT))? identifier correlationName?;
...
typeName: ((schemaName DOT)? identifier);

variableName: ((schemaName DOT)? identifier);

arrayTypeName: ((schemaName DOT)? identifier);

jarName: ((schemaName DOT)? identifier);

savepointName: identifier;

aliasName: (schemaName DOT)? identifier;

constraintName: identifier;

routineVersionID: (identifier | NUMERICLITERAL | INTEGERLITERAL) (
		DOT? (identifier | NUMERICLITERAL | INTEGERLITERAL)
	)*;

versionID: (identifier | NUMERICLITERAL | INTEGERLITERAL) (
		DOT? (identifier | NUMERICLITERAL | INTEGERLITERAL)
	)*;

indexName: (schemaName DOT)? identifier;

maskName: (schemaName DOT)? identifier;

permissionName: (schemaName DOT)? identifier;

procedureName: (
		(locationName DOT schemaName DOT)
		| (schemaName DOT)
	)? identifier;

sequenceName: (schemaName DOT)? identifier;
 ...
 */
triggerName: (schemaName DOT)? identifier;
...
seclabelName: identifier;

parameterName: identifier;

addressValue: NONNUMERICLITERAL;

jobnameValue: NONNUMERICLITERAL;

servauthValue: NONNUMERICLITERAL;

encryptionValue: NONNUMERICLITERAL;

bpName: identifier;

stogroupName: identifier;
...
keyLabelName: (identifier | NONNUMERICLITERAL);

functionName: (schemaName DOT)? identifier;

specificName: (schemaName DOT)? identifier;

hostLabel: (INTEGERLITERAL | identifier) (
		INTEGERLITERAL
		| (MINUS identifier)
	)*;

hostVariable:
	COLON (hostStructure DOT)? hostIdentifier (
		INDICATOR? COLON (nullIndicatorStructure DOT)? nullIndicator
	)?;

/*
 Making the MINUS optional because COBOL (at least) host variables
 can look pretty strange...
 A-001B1C2 and so forth.
 2025-03-19
 */
hostIdentifier: (INTEGERLITERAL | identifier) (
		INTEGERLITERAL
		| (MINUS* identifier)
	)*;

hostStructure: (INTEGERLITERAL | identifier) (
		INTEGERLITERAL
		| (MINUS* identifier)
	)*;

nullIndicator: (INTEGERLITERAL | identifier) (
		INTEGERLITERAL
		| (MINUS* identifier)
	)*;

nullIndicatorStructure: (INTEGERLITERAL | identifier) (
		INTEGERLITERAL
		| (MINUS* identifier)
	)*;

globalVariableName: ((schemaName DOT)? identifier1);

sqlParameterName: ((schemaName DOT)? identifier1);

sqlVariableName: ((schemaName DOT)? identifier1);

sqlConditionName: identifier1;

transitionVariableName: columnName;

synonym: identifier;

/*
 Trigger variables, global variables, SQL variables, all
 these conform to the pattern (schemaName
 DOT)? identifier.
 */
variable: ((schemaName DOT)? identifier) | hostVariable;

intoClause:
	INTO (variable | arrayElementSpecification) (COMMA variable)*;

correlationClause:
	AS? correlationName (
		LPAREN newColumnName (COMMA newColumnName)* RPAREN
	)?;
...
tableReference: (
 ...
 "The following sets of rules
		 are mutually left-recursive [tableReference, joinedTable]"
		 */
		| (
			(
				singleTableReference
				...
				| (
					(
						...
						| (LPAREN+ tableReference RPAREN+)
						| collectionDerivedTable
					) (INNER | ((LEFT | RIGHT | FULL) OUTER?))? JOIN tableReference ON joinCondition
				)
			) (
				(INNER | ((LEFT | RIGHT | FULL) OUTER?))? JOIN tableReference ON joinCondition
			)+
...
/*
 ...
 Martijn Rutte provided a test case and
 we found a solution 2023-05-23.
 */
dataChangeTableReference: (
		(
...
/**/
 ...
 (schemaName DOT)?
 (scalarFunction |
 aggregateFunction | regressionFunction | tableFunction | identifier)
 LPAREN
 ((expression | (TABLE
 tableName)) (COMMA (expression | (TABLE tableName)))*)?
 RPAREN
 ...
 : (
 AS?
 correlationName LPAREN columnName dataType (COMMA columnName dataType)* RPAREN
 )
 ;
...
collectionDerivedTable: (
		...
		)* RPAREN (WITH ORDINALITY)? correlationClause?
	);
...
fullJoinExpression: (
		(
			...
			| castFunction
			| (
				COALESCE LPAREN (columnName | castFunction) (
					COMMA (columnName | castFunction)
				)* RPAREN
			)
		)
	);

castFunction: (castSpecification);

ordinaryArrayExpression: (expression);

associativeArrayExpression: (expression);
...
groupByClause:
	GROUP BY (groupingExpression | groupingSets | superGroups);

havingClause: HAVING searchCondition;

groupingExpression: (expression (COMMA expression)*);

groupingSets: GROUPING SETS groupingSetsGroup;

groupingSetsGroup:
	LPAREN (groupingSetsGroup | groupingExpression | superGroups) (
		COMMA (
			groupingSetsGroup
			| groupingExpression
			| superGroups
...
selectColumns: (
		(
			expression ((operator expression) | INTEGERLITERAL)* (
				AS? (newColumnName | NONNUMERICLITERAL)
			)?
		)
		| (tableName DOT SPLAT)
		| unpackedRow
...
unpackedRow: (
		UNPACK LPAREN expression RPAREN DOT SPLAT AS LPAREN columnName dataType (
			COMMA columnName dataType
		)* RPAREN
	);
...
selectClause: (
		SELECT (ALL | DISTINCT)? (
			SPLAT
			| (selectColumns (COMMA selectColumns)*)
		)
	);
...
selectIntoStatement: (
		(
			...
			orderByClause? offsetClause? fetchClause? (
			isolationClause
			| skipLockedDataClause
			| optimizeClause
		)* querynoClause?
...
commonTableExpression:
	tableName (LPAREN columnName (COMMA columnName)* RPAREN)? AS LPAREN fullSelect RPAREN;

updateClause: (FOR UPDATE (OF columnName (COMMA columnName)*)?);

readOnlyClause: (FOR READ ONLY);

optimizeClause: OPTIMIZE FOR INTEGERLITERAL (ROW | ROWS);

isolationClause:
	WITH ((RR lockClause?) | (RS lockClause?) | CS | UR);

lockClause: (USE AND KEEP (EXCLUSIVE | UPDATE | SHARE) LOCKS);

skipLockedDataClause: (SKIP_ LOCKED DATA);

querynoClause: (QUERYNO INTEGERLITERAL);

scalarFullSelect: LPAREN fullSelect RPAREN;
...
sortKey: (columnName | INTEGERLITERAL | expression);

offsetClause: OFFSET INTEGERLITERAL (ROW | ROWS);

/*
 LIMIT syntax added per Martijn Rutte 2023-05-22.
 
 INTEGERLITERAL changed to (variable |
 INTEGERLITERAL) in first
 alternative per Martijn Rutte 2023-06-05.
 */
fetchClause: (
		FETCH (FIRST | NEXT) (variable | INTEGERLITERAL)? (
			ROW
			| ROWS
		) ONLY
	)
...
sqlidentifier: (SQLIDENTIFIER | NONNUMERICLITERAL);

/*
 Added TIMEOUT per Maarten van Haasteren 2024-06-04.
 
 Removed END_EXEC as it was getting caught up
 in the hostIdentifier,
 hostStructure, nullIndicator, nullIndicatorStructure changes
 to allow some
 of the more interesting COBOL constructs.
 2025-03-19
 */
sqlKeyword: (
		ABSOLUTE
